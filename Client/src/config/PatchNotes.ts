...
const PATCH_NOTES_RELEASES_UNSORTED: PatchNotesRelease[] = [
  {
    ...
    subtitle: 'Beta: safer automation, fortress logistics, and equipment choices',
    date: '2026-09-16',
    items: [
      { kind: 'added', text: 'Feature Stats shows final known scores from completed event runs in their related tabs, with local dates, ranks, filtering, and pagination while keeping unknown or active scores out of completed history.' },
      { kind: 'added', text: 'Auto Bird adds saved ignored-troop presets, scheduled preset changes, per-castle pause and resend controls, and a locked All Direwolves reserve for enabled Auto Fortress kingdom main castles. Turning Fortress off removes the automatic reserve and restores the saved manual quantity.' },
      { kind: 'added', text: 'Auto Fortress discovers outer-kingdom fortresses, balances and transfers purchased Direwolves across enabled main castles, optionally uses Time Skips for its own confirmed transfers, and attacks only freshly available targets within saved limits.' },
      { kind: 'added', text: 'Auto Booster can buy the current daily 2,500-ruby fortress-speed offer while preserving the configured ruby reserve and confirming the matching live effect.' },
      { kind: 'added', text: 'Auto Towers can use the seven-day Robber Baron Advisor, optionally activate it with an available dedicated token, and run bounded same-tower chains with complete formations and saved Time Skip limits.' },
