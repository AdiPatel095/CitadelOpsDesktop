...
const PATCH_NOTES_RELEASES_UNSORTED: PatchNotesRelease[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { kind: 'added', text: 'Hosted account owners can switch between stable and beta workers using the same account and license; the destination applies the latest canonical settings revision while each worker retains its own local files, logs, and history' },
      ...
      { kind: 'fixed', text: 'Auto Bird validates the actual troop payload before each support batch, fixing valid dispatches being rejected as empty while preserving the ten-type limit, castle pause, preset, focus, and troop-availability safeguards' },
      ...
      { kind: 'security', text: 'Worker switches require the source to stop and active safety locks to expire, verify the destination build and exact settings, and wait for game, metrics, and checkpoint readiness before unlocking controls; interrupted switches cannot start a second copy' },
