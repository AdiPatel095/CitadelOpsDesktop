...
import type {
	...
	BrowserInventory,
	GameServerEntry,
	PlayerHistoryRetentionOptionV1,
...
function retentionMagnitude(value: string): number | null {
	const normalized = value.trim().toLowerCase();
	if (normalized === 'none' || normalized === 'off' || normalized === 'disabled') return 0;
	if (normalized === 'unlimited' || normalized === 'forever') return Number.POSITIVE_INFINITY;
	const match = normalized.match(/^(\d+)(h|d|w|y)$/);
...
function allowedPlayerHistoryOptions(policy: PlayerHistoryRetentionV1 | null): PlayerHistoryRetentionOptionV1[] {
	...
	const maximumIndex = policy.options.findIndex((option) => option.value === policy.maximum);
	if (maximumIndex >= 0) return policy.options.slice(0, maximumIndex + 1);
	const maximum = retentionMagnitude(policy.maximum);
	if (maximum == null) return policy.options;
	return policy.options.filter((option) => {
		const magnitude = retentionMagnitude(option.value);
		return magnitude != null && magnitude <= maximum;
	});
}

function isRetentionReduction(
	current: string,
	next: string,
	options: readonly PlayerHistoryRetentionOptionV1[],
): boolean {
	if (!current || current === next) return false;
	const currentMagnitude = retentionMagnitude(current);
	const nextMagnitude = retentionMagnitude(next);
	if (currentMagnitude != null && nextMagnitude != null) return nextMagnitude < currentMagnitude;
  ...
  const [upgradeCoinThreshold, setUpgradeCoinThreshold] = useState<string>('0');
	...
	useEffect(() => {
		...
		void getPlayerHistoryRetention()
			...
			.finally(() => {
				if (active) setPlayerHistoryRetentionLoading(false);
			});
		return () => {
			active = false;
		};
	...
	const applyPlayerHistoryStoragePolicy = (nextRetention: string, nextIntervalSeconds: number) => {
		...
		void (async () => {
			try {
				...
				const preferenceSaved = error instanceof APIError && error.code === 'history_retention_apply_failed';
				setPlayerHistoryRetentionError(preferenceSaved
