...
export function AuthProvider({ children }: { children: ReactNode }) {
  const { connectionStatus, state, catalogs, submitIntent } = useCitadelAPI();
  ...
  const toggle = (
    ...
    const next = !enabled;
    setEnabled(next);
    void submitIntent('automation.set_enabled', { feature, enabled: next })
      .then((receipt) => {
        if (receipt.status === 'failed') setEnabled(enabled);
      })
  ...
  const triggerUpdate = (downloadUrl: string) => {
    ...
    void submitIntent('application.update', { downloadUrl })
      .then((receipt) => {
  ...
  const value = useMemo<AuthContextType>(() => ({
    ...
    toggleRecruitTroops: () => toggle('recruit_troops', recruitTroopsEnabled, setRecruitTroopsEnabled),
    toggleAutoTool: () => toggle('auto_tool', autoToolEnabled, setAutoToolEnabled),
    toggleAutoSceatRes: () => toggle('auto_sceat_resources', autoSceatResEnabled, setAutoSceatResEnabled),
    toggleAutoHospital: () => toggle('auto_hospital', autoHospitalEnabled, setAutoHospitalEnabled),
    toggleAutoTCI: () => toggle('auto_tci', autoTCIEnabled, setAutoTCIEnabled),
    toggleAutoBird: () => toggle('auto_bird', autoBirdEnabled, setAutoBirdEnabled),
    toggleAutoStation: () => toggle('auto_station', autoStationEnabled, setAutoStationEnabled),
    toggleAutoBeriWorld: () => toggle('auto_beri_world', autoBeriWorldEnabled, setAutoBeriWorldEnabled),
    sendMessage: (type, payload) => submit(type, isRecord(payload) ? payload : { value: payload }),
  }), [
    autoBeriWorldEnabled,
    autoBirdEnabled,
    autoHospitalEnabled,
