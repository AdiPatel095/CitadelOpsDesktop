...
const AttackSetupModal: React.FC<AttackSetupModalProps> = ({
  ...
  const troopInventoryIDs = useMemo(
    () => Array.from(new Set([
      ...Object.keys(inventory.troops).map(Number),
      ...Object.keys(allocations.troops).map(Number),
    ])).filter((id) => Number.isFinite(id) && id > 0),
    [allocations.troops, inventory.troops],
  );
  const displayedTroopStock = useMemo(
    () => draft.useTroopFamilies
      ? familyTotalsForIDs(inventory.troops, troopInventoryIDs, troops)
      : inventory.troops,
    [draft.useTroopFamilies, inventory.troops, troopInventoryIDs, troops],
  );
  const displayedTroopAllocations = useMemo(
    () => draft.useTroopFamilies
      ? familyTotalsForIDs(allocations.troops, troopInventoryIDs, troops)
      : allocations.troops,
    [allocations.troops, draft.useTroopFamilies, troopInventoryIDs, troops],
  );
  const inventoryIssues = useMemo(
    () => findInventoryIssues(allocations, inventory, Boolean(draft.useTroopFamilies), troops),
    [allocations, draft.useTroopFamilies, inventory, troops]
  );
        ...
        <section className="flex flex-col gap-4" aria-label="Attack waves">
          {draft.waves.map((wave, waveIndex) => (
            <WaveEditorCard
              ...
              toolItems={toolItems}
              troopStock={displayedTroopStock}
              toolStock={inventory.tools}
              troopAllocations={displayedTroopAllocations}
...
function findInventoryIssues(
  ...
  if (useTroopFamilies) {
    const requestedByFamily = new Map<string, { itemId: number; requested: number; ids: number[] }>();
    for (const [rawID, requested] of Object.entries(allocations.troops)) {
      const itemId = Number(rawID);
      const ids = unitUpgradeFamily(itemId, troopMetadata)?.ids ?? [itemId];
      const key = ids.join(',');
      const current = requestedByFamily.get(key);
      if (current) current.requested += requested;
      else requestedByFamily.set(key, { itemId, requested, ids });
    }
    for (const request of requestedByFamily.values()) {
      const stock = request.ids.reduce((total, id) => total + (inventory.troops[id] ?? 0), 0);
      if (request.requested > stock) {
        issues.push({ kind: 'troop', itemId: request.itemId, requested: request.requested, stock });
      }
    }
  } else {
    for (const [rawID, requested] of Object.entries(allocations.troops)) {
      const itemId = Number(rawID);
      const stock = inventory.troops[itemId] ?? 0;
      if (requested > stock) issues.push({ kind: 'troop', itemId, requested, stock });
    }
  }
  for (const [rawID, requested] of Object.entries(allocations.tools)) {
    const itemId = Number(rawID);
    const stock = inventory.tools[itemId] ?? 0;
    if (requested > stock) issues.push({ kind: 'tool', itemId, requested, stock });
  }
  return issues;
}

function familyTotalsForIDs(
  ...
  unitIDs.forEach((unitID) => {
    const ids = unitUpgradeFamily(unitID, troopMetadata)?.ids ?? [unitID];
    totals[unitID] = ids.reduce((total, id) => total + (values[id] ?? 0), 0);
  });
  return totals;
}
