  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        const rangeSeconds = primarySeconds == null || troopSeconds == null
          ? 'all'
          : String(Math.max(primarySeconds, troopSeconds));
        const response = await runtimeFetch(`/api/v2/history/player-tracker?rangeSeconds=${rangeSeconds}`, { cache: 'no-store', signal: controller.signal });
