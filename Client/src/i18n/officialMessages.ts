...
export function officialMessageKeysFor(descriptor?: LocalizedMessage): string[] {
  return [...new Set([descriptor,...descriptor?.context ?? []].flatMap(item=>item ? [item.officialKey,...Object.values(item.gameParams ?? {}).map(noun=>noun.key)].filter((key):key is string=>!!key) : []))].sort();
}
