...
export function saveAutoStormBlueprint(
  value: unknown,
  target: BuildingTargetCaptureResponse,
  now = new Date().toISOString(),
): AutoStormBlueprintDocument {
  const document = parseAutoStormBlueprintDocument(value);
  const id = target.mode === 'functional' ? 'storm-functional'
    : target.mode === 'layout' || target.mode === 'buildings' ? 'storm-layout'
      : 'storm-exact';
...
export function activateAutoStormBlueprint(value: unknown, id: string): AutoStormBlueprintDocument {
  const document = parseAutoStormBlueprintDocument(value);
  const activeId = id.trim();
  if (activeId && !document.blueprints[activeId]) throw new Error(`Storm blueprint ${activeId} does not exist`);
