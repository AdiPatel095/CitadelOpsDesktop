...
export const AutoToolSettingsModal: React.FC<AutoToolSettingsModalProps> = ({ isOpen, onClose, onOpenFeatureSchedule }) => {
  ...
  const currentItemsForScope = (scope: AutoToolItemScope) => {
    if (scope.type === 'global') return settings.globalItems;
    return settings.castles[scope.castleId]?.items ?? [];
  };

  const allowedToolIdsForScope = (scope: AutoToolItemScope): number[] | undefined => {
    ...
    if (scope.type === 'castle') {
      ...
      return queueableIDsForCastle(queueableCatalog, scope.castleId, 'toolIds');
    }
    ...
    if (enabledCastleIDs.length > 0) {
      return queueableIDsForCastles(queueableCatalog, enabledCastleIDs, 'toolIds', 'intersection');
    }
    if (knownCastleIDs.length > 0) {
      return queueableIDsForCastles(queueableCatalog, knownCastleIDs, 'toolIds', 'union');
    }
    return undefined;
  };

  const updateItemsForScope = (scope: AutoToolItemScope, items: AutoToolItem[]) => {
    setSettings(prev => {
  ...
  const handleAddItem = async (scope: AutoToolItemScope, title: string) => {
    ...
    const result = await showToolPicker({
      ...
      allowedToolIds: allowedToolIdsForScope(scope),
    });
  ...
  const openEditModal = (scope: AutoToolItemScope, item: AutoToolItem) => {
    setEditingTool({ scope, item });
  };
  ...
  const deleteFromEditModal = () => {
    ...
    updateItemsForScope(
      editingTool.scope,
      currentItemsForScope(editingTool.scope).filter(item => item.id !== editingTool.item.id),
    );
    closeEditModal();
  };
  ...
  const updateCheckIntervalMinutes = (value: string) => {
    ...
    setSettings(prev => ({
      ...prev,
      checkIntervalSec: autoToolCheckIntervalMinutesToSec(raw === '' ? DEFAULT_AUTO_TOOL_CHECK_INTERVAL_MIN : Number(raw)),
    }));
  };

  const handleSave = () => {
    setIsSaving(true);
    const sent = persistAutoToolSettings(settings);
    setIsSaving(false);
    if (sent) onClose();
  };
  ...
  const eligibleCastles = queueableCatalogLoaded
    ? castles.filter((castle) => queueableCastleEligible(queueableCatalog, castle.id, 'toolIds'))
    : castles;
  ...
  const autoToolSchedule = featureSchedules.autoTool;
  const autoToolScheduleEnabled = !!autoToolSchedule?.enabled;
  const autoToolUsesScheduledTools = !!(autoToolScheduleEnabled && autoToolSchedule?.slotOptionsEnabled);
  const shouldShowSharedToolPanel = isGlobalMode && !autoToolUsesScheduledTools;
  const globalHasToolSourcePanel = shouldShowSharedToolPanel || autoToolUsesScheduledTools;

  const renderAutoTools = (
    items: AutoToolItem[],
    scope: AutoToolItemScope,
    addLabel: string,
    addTitle: string,
  ) => {
    if (items.length === 0) {
      ...
      <div className="flex flex-wrap gap-4 content-start">
        {items.map((item) => (
          <button
            ...
            type="button"
            className="group/tool relative flex w-[5.75rem] flex-col items-center gap-2 rounded-global border border-border-base bg-bg-card/70 p-3 text-center shadow-sm transition-transform hover:-translate-y-1 hover:border-primary/45"
            title={toolName(item.id)}
            onClick={() => openEditModal(scope, item)}
          >
            <ToolImage toolId={item.id} size={66} showLevel={true} className="rounded-xl" />
            <span className="line-clamp-2 min-h-[2rem] text-xs font-bold leading-tight text-text-main">
              {toolName(item.id)}
            </span>
            <span className="pointer-events-none absolute inset-0 flex items-center justify-center rounded-global bg-black/30 opacity-0 transition-opacity group-hover/tool:opacity-100">
  ...
  const toolIDFromScheduleSlot = (slot: WeeklySchedule['slots'][number]) => {
    const raw = slot.options?.toolID;
      ...
      <div className="rounded-global border border-primary/20 bg-primary/5 p-3">
        ...
        {visibleSlots.length === 0 ? (
          <div className="rounded-global border border-dashed border-border-base bg-bg-card/45 px-4 py-3 text-xs font-semibold text-text-muted">
            {options.emptySlotsLabel ?? 'No scheduled tool slots'}
          </div>
        ) : (
          <div className="grid gap-2">
            {visibleSlots.map((slot) => {
              const toolID = toolIDFromScheduleSlot(slot) ?? options.fallbackToolID ?? null;
              const label = toolID
                ? toolName(toolID)
                : options.emptyToolLabel ?? 'No tool selected';
              ...
              return (
                ...
                >
                  {toolID ? (
                    <ToolImage toolId={toolID} size={34} showLevel={true} className="shrink-0 rounded-lg" />
                  ) : (
  ...
  const renderSharedToolPanel = (className = '') => {
    const sharedToolID = settings.globalItems[0]?.id ?? null;
    ...
    return (
      <Card variant="solid" className={`liquid-prominent-header-card flex flex-col ${className}`}>
        <CardHeader className="liquid-card-header-prominent flex flex-row items-center justify-between gap-3">
          <div className="min-w-0">
            <CardTitle className="text-base">Shared Tool</CardTitle>
            <p className="mt-1 text-xs text-text-muted">
              {autoToolScheduleEnabled
                ? 'Calendar windows control when this shared tool is queued.'
                : 'Shared tool can be overridden by the Auto Tool calendar.'}
            </p>
          </div>
          <div className="flex shrink-0 items-center gap-2">
            {autoToolScheduleEnabled && (
              <Badge variant="primary">
                {autoToolSchedule?.slots.length ?? 0} slot{autoToolSchedule?.slots.length === 1 ? '' : 's'}
              </Badge>
            )}
            <Button
              variant="outline"
              size="sm"
              onClick={() => onOpenFeatureSchedule('autoTool', 'Auto Tool')}
              leftIcon={<CalendarDays className="h-4 w-4" />}
            >
              Calendar
            </Button>
        ...
        <CardContent className="liquid-prominent-header-content flex flex-1 flex-col gap-4 p-5">
          ...
          {autoToolScheduleEnabled && autoToolSchedule && (
            renderScheduleSlots(autoToolSchedule, {
              title: 'Scheduled Windows',
              description: 'All enabled castles queue the shared tool during these periods.',
              editTitle: 'Auto Tool calendar',
              onEdit: () => onOpenFeatureSchedule('autoTool', 'Auto Tool'),
              hideEditButton: true,
              fallbackToolID: sharedToolID,
              emptyToolLabel: 'Shared tool not selected',
    ...
    <Card variant="solid" className={`liquid-prominent-header-card flex flex-col ${className}`}>
      <CardHeader className="liquid-card-header-prominent flex flex-row items-center justify-between gap-3">
        <div className="min-w-0">
          ...
          <p className="mt-1 text-xs text-text-muted">Scheduled tools replace the shared tool picker.</p>
        </div>
        <div className="flex shrink-0 items-center gap-2">
          <Badge variant="primary">
      ...
      <CardContent className="liquid-prominent-header-content flex-1 p-5">
        {renderScheduleSlots(schedule, {
          title: 'Time Slots',
          description: 'All enabled castles use these calendar tool slots.',
          editTitle: 'Auto Tool calendar',
          onEdit: () => onOpenFeatureSchedule('autoTool', 'Auto Tool'),
          hideEditButton: true,
        })}
  ...
  const renderGlobalToolSourcePanel = (className = '') => {
    if (autoToolUsesScheduledTools && autoToolSchedule) {
      return renderGlobalSchedulePanel(autoToolSchedule, className);
    }
    if (shouldShowSharedToolPanel) {
      return renderSharedToolPanel(className);
    }
    return null;
  };
        ...
        <div className="flex min-h-[6.75rem] w-full items-center gap-4 rounded-global border border-dashed border-border-base bg-bg-card/35 p-4">
          ...
          <div className="min-w-0">
            ...
            <p className="mt-2 max-w-sm text-sm leading-relaxed text-text-muted">
              Connect and refresh castle data before configuring Auto Tool, or verify the castle has a siege or defense workshop.
            </p>
          </div>
        </div>
      </CardContent>
    ...
    <Card variant="solid" className={`liquid-prominent-header-card flex flex-col ${className}`}>
      <CardHeader className="liquid-card-header-prominent flex flex-row items-center justify-between gap-3">
        <div className="min-w-0">
          ...
          <p className="mt-1 text-xs text-text-muted">Enable Auto Tool coverage for each castle.</p>
        </div>
  ...
  return (
    <>
        ...
        title={
          <div className="scheduler-modal-title">
            ...
            <span className="flex min-w-0 flex-col">
              <span className="scheduler-modal-title-text">Auto Tool Settings</span>
        ...
        <div className={`recruit-modal-shell mx-auto flex w-full flex-col gap-5 overflow-visible pb-2 ${isGlobalMode ? 'max-w-6xl' : 'max-w-[min(1840px,98vw)]'}`}>
          <div className="grid grid-cols-1 gap-4 xl:grid-cols-[minmax(17rem,0.8fr)_minmax(22rem,1.15fr)_minmax(11rem,0.5fr)]">
            <Card variant="solid" className="liquid-prominent-header-card">
              ...
              <CardContent className="liquid-prominent-header-content p-5">
                <Input
                  type="text"
                  value={autoToolCheckIntervalSecToMinutes(settings.checkIntervalSec).toLocaleString()}
                ...
                <p className="mt-2 text-[11px] font-medium text-text-muted">
                  Minimum {MIN_AUTO_TOOL_CHECK_INTERVAL_MIN.toLocaleString()} minute. Default is {DEFAULT_AUTO_TOOL_CHECK_INTERVAL_MIN.toLocaleString()} minutes.
                </p>
              </CardContent>
            </Card>
            ...
            <Card variant="solid" className="liquid-prominent-header-card">
              <CardHeader className="liquid-card-header-prominent">
                <div>
                  <CardTitle className="text-base">Tool Mode</CardTitle>
                  <p className="mt-1 text-xs text-text-muted">
                    Choose one shared tool list or separate lists and schedules per castle.
                  </p>
                </div>
              </CardHeader>
              <CardContent className="liquid-prominent-header-content p-5">
                  ...
                  options={[
                    {
                      ...
                      title: 'Use one tool for all enabled castles',
                    },
                    {
                      value: 'perCastle',
                      label: 'Specific per Castle',
                      icon: <Settings className="h-4 w-4" />,
                      title: 'Use separate tools and schedules per castle',
                    },
                  ]}
                  fullWidth
                />
            ...
            <Card variant="solid" className="liquid-prominent-header-card">
              <CardHeader className="liquid-card-header-prominent">
                <div>
                  ...
                  <p className="mt-1 text-xs text-text-muted">Castles selected for Auto Tool.</p>
                </div>
              </CardHeader>
          ...
          {isGlobalMode ? (
            eligibleCastles.length === 0 ? (
              globalHasToolSourcePanel ? (
                <div className="grid gap-4 lg:grid-cols-[minmax(0,1.15fr)_minmax(18rem,0.85fr)]">
                  {renderGlobalToolSourcePanel('h-full')}
              ...
              <div className={`grid gap-4 ${globalHasToolSourcePanel ? 'lg:grid-cols-[minmax(0,1fr)_minmax(20rem,0.75fr)]' : ''}`}>
                {renderGlobalToolSourcePanel('h-full')}
            ...
            <>
              <div className="grid w-full auto-rows-max grid-cols-1 gap-5 lg:grid-cols-2 2xl:grid-cols-3">
                {eligibleCastles.map((castle) => {
                  const castleId = castle.id.toString();
                  const castleScheduleID = autoToolCastleScheduleID(castle.id);
                  const castleSchedule = featureSchedules[castleScheduleID];
                  const scheduledToolSchedule = !isGlobalMode && castleSchedule?.enabled && castleSchedule.slotOptionsEnabled ? castleSchedule : null;
                  const castleUsesScheduledTools = !!scheduledToolSchedule;
                  const castleSettings = settings.castles[castleId] ?? { enabled: false, items: [] };
                        ...
                        <div className="flex shrink-0 items-center gap-2">
                          {!isGlobalMode && (
                            <Button
                              variant="ghost"
                              size="icon"
                              onClick={() => onOpenFeatureSchedule(castleScheduleID, `Auto Tool - ${castle.name}`)}
                              title={`${castle.name} Auto Tool schedule`}
                            >
                      ...
                      <CardContent className="liquid-prominent-header-content flex flex-1 flex-col gap-4 p-5">
                        <div className="flex flex-wrap items-center justify-between gap-2 text-xs text-text-muted">
                          <span>
                            {isGlobalMode
                              ? 'Uses shared tool'
                              : castleUsesScheduledTools
                                ? 'Uses scheduled tool slots'
                                : 'Uses castle-specific tools'}
                          </span>
                          {scheduledToolSchedule ? (
                            <Badge variant="primary">
                              {scheduledToolSchedule.slots.length} slot{scheduledToolSchedule.slots.length !== 1 ? 's' : ''}
                            </Badge>
                          ) : hasItems && (
                            <Badge variant="primary">
                              {displayedItems.length} tool{displayedItems.length !== 1 ? 's' : ''}
                            </Badge>
                          )}
                        </div>
                        {isGlobalMode ? (
                          ...
                          renderCastleScheduleSlots(scheduledToolSchedule, castle)
                        ) : (
                          renderAutoTools(
                            castleSettings.items,
                            { type: 'castle', castleId },
                            'Select tool',
                            `Select tool - ${castle.name}`,
                          )
                        )}
                      </CardContent>
                    </Card>
      ...
      <Modal
        ...
        maxWidth="sm"
        title={editingTool ? toolName(editingTool.item.id) : 'Tool'}
        footer={
          <>
      ...
      >
        <div className="flex flex-col items-center gap-6 py-4">
          {editingTool && (
            <ToolImage toolId={editingTool.item.id} size={80} showLevel={true} className="rounded-2xl shadow-lg" />
          )}
