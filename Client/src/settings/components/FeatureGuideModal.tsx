...
type Feature = 'autoTower' | 'autoBird' | 'autoStation' | 'autoFortress' | 'autoInvasion';
...
export function FeatureGuideModal({ feature, isOpen, onClose, showAdvisor = true }: { feature: Feature; isOpen: boolean; onClose: () => void; showAdvisor?: boolean }) {
  ...
  const pack = (feature === 'autoFortress' && !('autoFortress' in selectedPack) || feature === 'autoInvasion' && !('autoInvasion' in selectedPack)) ? englishGuidePack : selectedPack;
  const locale = pack === englishGuidePack ? 'en' : selectedLocale;
  const [previewStepId, setPreviewStepId] = useState<string | null>(null);
  useEffect(() => { if (!isOpen) setPreviewStepId(null); }, [isOpen]);
  const source = feature === 'autoTower' ? towerSource : feature === 'autoBird' ? birdSource : feature === 'autoStation' ? stationSource : feature === 'autoFortress' ? fortressSource : invasionSource;
  ...
  const title = feature === 'autoTower' ? pack.ui.towerGuideTitle : feature === 'autoBird' ? pack.ui.birdGuideTitle : feature === 'autoStation' ? pack.ui.stationGuideTitle : feature === 'autoFortress' ? pack.ui.fortressGuideTitle : pack.ui.invasionGuideTitle;
  const intro = feature === 'autoTower' ? pack.ui.towerGuideIntro : feature === 'autoBird' ? pack.ui.birdGuideIntro : feature === 'autoStation' ? pack.ui.stationGuideIntro : feature === 'autoFortress' ? pack.ui.fortressGuideIntro : pack.ui.invasionGuideIntro;
  const previewTitle = feature === 'autoTower' ? pack.ui.towerPreviewTitle : feature === 'autoBird' ? pack.ui.birdPreviewTitle : feature === 'autoStation' ? pack.ui.stationPreviewTitle : feature === 'autoFortress' ? pack.ui.fortressPreviewTitle : pack.ui.invasionPreviewTitle;
  return <>
        ...
        <ol className="space-y-6">
          {steps.map((step, index) => {
            ...
            return <li key={step.id}>
              ...
              {step.image && 'image' in content && content.image && <figure className="mt-3" style={{ maxWidth: locale === 'en' && feature !== 'autoFortress' && feature !== 'autoInvasion' ? step.image.width : 680 }}>
                <button type="button" onClick={() => setPreviewStepId(step.id)} aria-label={pack.ui.enlargePicture}
                  ...
                  {locale === 'en' && feature !== 'autoFortress' && feature !== 'autoInvasion' ? <img src={step.image.src} alt={content.image.alt} width={step.image.width} height={step.image.height} loading="lazy" className="h-auto w-full" /> :
                ...
                <figcaption className="mt-2 text-xs leading-relaxed text-text-muted">{(locale !== 'en' || feature === 'autoFortress' || feature === 'autoInvasion') && <strong>{pack.ui.illustrativeExample}. </strong>}{content.image.caption}</figcaption>
              </figure>}
            </li>;
          })}
        </ol>
    ...
    <Modal isOpen={isOpen && !!previewStep} onClose={() => setPreviewStepId(null)} title={previewTitle} contentLang={locale} contentDir={locale === 'ar' ? 'rtl' : 'ltr'} closeLabel={pack.ui.backToGuide} maxWidth="full"
      ...
      {previewStep && (() => {
        ...
        return <div lang={locale} dir={locale === 'ar' ? 'rtl' : 'ltr'} className="overflow-auto" tabIndex={0} role="region" aria-label={pack.ui.fullSizePicture}>
          {locale === 'en' && feature !== 'autoFortress' && feature !== 'autoInvasion' ? <img src={previewStep.image.src} alt={content.image.alt} width={previewStep.image.width} height={previewStep.image.height} className="h-auto max-w-none" style={{ width: previewStep.image.width }} /> :
