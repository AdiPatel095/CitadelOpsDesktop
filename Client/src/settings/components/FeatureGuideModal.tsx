...
import stormSource from '../../config/autoStormGuide.json';
...
type Feature = 'autoTower' | 'autoBird' | 'autoStation' | 'autoFortress' | 'autoInvasion' | 'autoNomad' | 'autoAdvisor' | 'autoKhan' | 'autoBeri' | 'autoStorm';
...
export function FeatureGuideModal({ feature, isOpen, onClose, showAdvisor = true }: { feature: Feature; isOpen: boolean; onClose: () => void; showAdvisor?: boolean }) {
  ...
  const pack = (feature === 'autoFortress' && !('autoFortress' in selectedPack) || feature === 'autoInvasion' && !('autoInvasion' in selectedPack) || feature === 'autoNomad' && !('autoNomad' in selectedPack) || feature === 'autoAdvisor' && !('autoAdvisor' in selectedPack) || feature === 'autoKhan' && !('autoKhan' in selectedPack) || feature === 'autoBeri' && !('autoBeri' in selectedPack) || feature === 'autoStorm' && !('autoStorm' in selectedPack)) ? englishGuidePack : selectedPack;
  const locale = pack === englishGuidePack ? 'en' : selectedLocale;
  const [previewStepId, setPreviewStepId] = useState<string | null>(null);
  useEffect(() => { if (!isOpen) setPreviewStepId(null); }, [isOpen]);
  const source = feature === 'autoTower' ? towerSource : feature === 'autoBird' ? birdSource : feature === 'autoStation' ? stationSource : feature === 'autoFortress' ? fortressSource : feature === 'autoInvasion' ? invasionSource : feature === 'autoNomad' ? nomadSource : feature === 'autoAdvisor' ? advisorSource : feature === 'autoBeri' ? beriSource : feature === 'autoStorm' ? stormSource : khanSource;
    ...
    <Modal isOpen={isOpen && !!previewStep} onClose={() => setPreviewStepId(null)} title={previewTitle} contentLang={locale} contentDir={locale === 'ar' ? 'rtl' : 'ltr'} closeLabel={pack.ui.backToGuide} maxWidth="full"
      ...
      {previewStep && (() => {
        ...
        return <div lang={locale} dir={locale === 'ar' ? 'rtl' : 'ltr'} className="overflow-auto" tabIndex={0} role="region" aria-label={pack.ui.fullSizePicture}>
          {locale === 'en' && feature !== 'autoFortress' && feature !== 'autoInvasion' && feature !== 'autoNomad' && feature !== 'autoAdvisor' && feature !== 'autoKhan' && feature !== 'autoBeri' && feature !== 'autoStorm' ? <img src={previewStep.image.src} alt={content.image.alt} width={previewStep.image.width} height={previewStep.image.height} className="h-auto max-w-none" style={{ width: previewStep.image.width }} /> :
