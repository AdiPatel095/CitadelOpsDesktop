...
Status: partial implementation, not a translated release. Baseline: `e77ed022c2f926ae3509527fdabf54dca9e22586`. Current source has **1,879 typed keys: 1,801 custom and 78 official routes**. Each of 25 non-English UI packs contains **293 authored entries**, leaving **1,508 custom keys missing per locale**. Complete authored groups are equipment (63), activity (32), and battle presentation (66), and event/history presentation (41); those group counts do not certify their entire screens. Native-speaker review remains pending. English fallbacks are not credited as translations; legitimate language-neutral numeric/ID templates preserve their semantics.
...
The latest inventory has **6,701 unreviewed conservative candidates**, 47 source-bound reviewed records and 10,065 structural exclusions. Candidate count is not a count of visible messages: property selectors, raw identity values and other dataflow candidates still need review. Run `npm run check:localization` from Client to regenerate the inventory and exact coverage counts.
