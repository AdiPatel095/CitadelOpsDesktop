...
Status: partial implementation, not a translated release. Baseline: e77ed022c2f926ae3509527fdabf54dca9e22586. There are 808 typed source keys: 783 custom messages and 25 verified official game-key routes. Each of the 25 non-English catalogs contains 92 model-authored custom translations; 691 custom messages per locale remain untranslated. Human linguistic review is pending. English fallback is not credited as translated.

The source-aware static migration assigns 729 standalone JSX sinks across 64 files to 712 file-context keys. Of these, 21 keys / 28 sinks use reviewed official game terms. `static-migrations.json` records the source location, route, and explicit rich/dynamic/technical exclusions. `ui.en.json` is generated by the strict gate for translation tooling; source-of-truth keys remain in `messages.ts` and `sourceMessages.ts`.
