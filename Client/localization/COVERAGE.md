...
Status: partial implementation, not a translated release. Baseline: `e77ed022c2f926ae3509527fdabf54dca9e22586`. Current source has **1,814 typed keys: 1,739 custom and 75 official routes**. Each of 25 non-English UI packs contains **244 authored entries**, leaving **1,495 custom keys missing per locale**. Complete authored groups are equipment (63), activity (32), and battle presentation (58); those group counts do not certify their entire screens. Native-speaker review remains pending. English fallbacks are not credited as translations; legitimate language-neutral numeric/ID templates preserve their semantics.

Backend catalogs contain 25 complete 289-key packs. Server catalogs contain 148 of 3,040 entries per locale, synchronized from approved `ecf71a1c905fd601951eb2d2b2fcbbcd56cfd6e6`. Final approved Server source integration is outstanding. The strict gate compares the copied English catalog against the actual Server catalog in this checkout, so internally consistent stale copies cannot pass.

## Source inventory

The latest inventory has **6,715 unreviewed conservative candidates**, 43 source-bound reviewed records and 9,996 structural exclusions. Candidate count is not a count of visible messages: property selectors, raw identity values and other dataflow candidates still need review. Run `npm run check:localization` from Client to regenerate the inventory and exact coverage counts.
...
| Battle reports | New presentation group covers result/filter labels, aggregate counts, dates, plural summaries and whole item tooltips. Remaining lane/location fallbacks, effect presentation and older source-assigned labels remain. |
