...
The latest inventory has **6,693 unreviewed conservative candidates**, 47 source-bound reviewed records and 10,065 structural exclusions. Candidate count is not a count of visible messages: property selectors, raw identity values and other dataflow candidates still need review. Run `npm run check:localization` from Client to regenerate the inventory and exact coverage counts.
