...
func (engine *Engine) execute(prepared *preparedSubmission) Receipt {
	...
	for {
		...
		attemptContext = context.WithValue(attemptContext, operationEvidenceContextKey{}, evidenceBuffer)
			...
			for err == nil {
				...
				if evidence := evidenceBuffer.drain(); len(evidence) > 0 {
					...
					if persistErr := engine.update(receipt); persistErr != nil {
						release()
						return engine.persistenceFailure(receipt, fmt.Errorf("persist operation evidence: %w", persistErr))
					}
				}
...
func stepResumeKey(step Step) string {
	return strings.Join([]string{
		...
		fmt.Sprint(step.ResponseIdentity.CastleID),
		func() string {
...
func (engine *Engine) executeStep(ctx context.Context, afterRevision uint64, step Step) (*CommandExchange, error) {
	...
	metadata.ResponseIdentity = step.ResponseIdentity
