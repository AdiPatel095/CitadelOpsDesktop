...
func TestAutoKhanPolicyBlocksAnUnsafeCurrentRun(t *testing.T) {
	...
	snapshot.State.Khan = State.KhanState{
		...
		Launches: []State.KhanLaunchState{
			{CommanderID: 1, MovementID: 10, ArrivesAt: now.Add(2 * time.Minute)},
			{CommanderID: 2, MovementID: 11, ArrivesAt: now.Add(time.Minute)},
...
func TestAutoKhanPolicyRecoversAfterUnsafeArrivalsFinish(t *testing.T) {
	now := time.Date(2026, 7, 14, 15, 0, 0, 0, time.UTC)
	snapshot := autoKhanPolicySnapshot(t, now, 2)
	snapshot.State.Khan = State.KhanState{
		RunID: "unsafe", SourceCastleID: 2, MainCastleID: 1, KingdomID: 0, TargetX: 210, TargetY: 942,
		AttacksLaunched: 2, VictoriesConfirmed: 2,
		Launches: []State.KhanLaunchState{
			{CommanderID: 1, MovementID: 10, ArrivesAt: now.Add(-time.Minute)},
			{CommanderID: 2, MovementID: 11, ArrivesAt: now.Add(-2 * time.Minute)},
		}, Taunts: map[State.MovementID]State.KhanTauntState{},
		SafetyError: "commander 2 arrived before commander 1",
	}
	decision, err := NewAutoKhanPolicy().Evaluate(t.Context(), snapshot)
	if err != nil || decision.Request == nil || decision.Request.Name != "khan.attack" {
		t.Fatalf("finished inversion recovery decision: %#v err=%v", decision, err)
	}
}

func TestAutoKhanPolicyWaitsWhenNewlyAvailableFasterCommanderWouldOvertake(t *testing.T) {
	now := time.Date(2026, 8, 29, 11, 45, 46, 740198696, time.UTC)
	snapshot := autoKhanPolicySnapshot(t, now, 2)
	snapshot.State.Commanders[1] = State.CommanderState{
		ID: 1, Name: "Loot Speed 16", Available: false,
		Equipment: map[string]State.EquipmentInstanceID{}, Gems: map[string]State.GemInstanceID{},
	}
	snapshot.State.Commanders[2] = State.CommanderState{
		ID: 2, Name: "Super", Available: true,
		Equipment: map[string]State.EquipmentInstanceID{"1": 200}, Gems: map[string]State.GemInstanceID{},
	}
	snapshot.State.Inventory.Equipment[200] = State.EquipmentInstance{
		ID: 200, Effects: State.EquipmentEffects{{DefinitionID: 900, Values: []float64{100}}},
	}
	...
	if err != nil || decision.Request != nil || decision.Status != "waiting" ||
		!strings.Contains(decision.Detail, "before launching a faster available commander") {
		t.Fatalf("faster returner decision: %#v err=%v", decision, err)
	}
	if !decision.NextCheckAt.Equal(previousArrival) || decision.Metrics["arrivalOrderWait"] != 1 {
		t.Fatalf("faster returner wait bounds: %#v", decision)
	}

	snapshot.Now = previousArrival.Add(time.Nanosecond)
	decision, err = NewAutoKhanPolicy().Evaluate(t.Context(), snapshot)
	if err != nil || decision.Request == nil || decision.Request.Name != "khan.attack" {
		t.Fatalf("post-arrival faster commander decision: %#v err=%v", decision, err)
	}
}
...
func TestAutoKhanAttackPolicyClearsUnreportedCooldownImmediatelyBeforeCRA(t *testing.T) {
	...
	if err != nil || decision.Request == nil ||
		...
		!strings.Contains(decision.Detail, "immediately before the next attack") {
...
func TestAutoKhanCooldownPolicySkipsCooldownAfterConfirmedReportAndReping(t *testing.T) {
	...
	if err != nil || attackDecision.Request != nil || attackDecision.Status != "cooldown" ||
		!strings.Contains(attackDecision.Detail, "waits for its battle report") {
...
func TestAutoKhanMissingPresetUnitsPausesOnlyTheCRALaunchCursor(t *testing.T) {
	...
	decision, err := NewAutoKhanPolicy().Evaluate(t.Context(), snapshot)
	if err != nil || decision.Request != nil || decision.Status != "waiting" ||
		!strings.Contains(decision.Detail, "Khan attacks are paused") ||
...
func TestAutoKhanMaxRageChainStopsAttacksButContinuesTaunts(t *testing.T) {
	now := time.Date(2026, 8, 29, 16, 0, 0, 0, time.UTC)
	snapshot := autoKhanPolicySnapshot(t, now, 2)
	setAutoKhanPolicySetting(t, &snapshot, "maxRageChain", 2)
	activity := snapshot.State.EventScores.ActivityByEvent[autoKhanEventID]
	activity.KhanDefense.Launches = 2
	snapshot.State.EventScores.ActivityByEvent[autoKhanEventID] = activity

	attackDecision, err := NewAutoKhanPolicy().Evaluate(t.Context(), snapshot)
	if err != nil || attackDecision.Request != nil || attackDecision.Status != "idle" ||
		!strings.Contains(attackDecision.Detail, "2 / 2") || attackDecision.Metrics["rageChainTaunts"] != 2 {
		t.Fatalf("capped attack decision: %#v err=%v", attackDecision, err)
	}
	...
	snapshot.State.Khan.PlayerTotalRage = 1740
	snapshot.State.Khan.RageObservedAt = now
	impactAt := now.Add(time.Minute)
	snapshot.State.Khan.Taunts[90] = State.KhanTauntState{MovementID: 90, ObservedAt: now, ImpactAt: impactAt}
	rageDecision, err := NewAutoKhanRagePolicy().Evaluate(t.Context(), snapshot)
	if err != nil || rageDecision.Request == nil || rageDecision.Request.Name != "khan.taunt" ||
		rageDecision.Status != "taunting" || rageDecision.Metrics["rageChainTaunts"] != 2 {
		t.Fatalf("rage threshold blocked the continuing taunt chain: %#v err=%v", rageDecision, err)
	}

	snapshot.State.Khan.LastTauntTriggeredAt = now.Add(time.Second)
	defenseDecision, err := NewAutoKhanDefensePolicy().Evaluate(t.Context(), snapshot)
	if err != nil || defenseDecision.Request == nil || defenseDecision.Request.Name != "defense.preset.apply" {
		t.Fatalf("rage cap blocked defense recovery: %#v err=%v", defenseDecision, err)
	}
}

func TestAutoKhanMaxRageChainMaintainsCooldowns(t *testing.T) {
	now := time.Date(2026, 8, 29, 16, 3, 0, 0, time.UTC)
	snapshot := autoKhanPolicySnapshot(t, now, 2)
	setAutoKhanPolicySetting(t, &snapshot, "maxRageChain", 1)
	activity := snapshot.State.EventScores.ActivityByEvent[autoKhanEventID]
	activity.KhanDefense.Launches = 1
	snapshot.State.EventScores.ActivityByEvent[autoKhanEventID] = activity

	snapshot.State.NomadCamps.Cooldowns["0:210:942"] = State.NomadCampCooldownState{
		KingdomID: 0, X: 210, Y: 942, LastSuccessfulBattleAt: now, PendingCooldownRefresh: true,
	}
	decision, err := NewAutoKhanPolicy().Evaluate(t.Context(), snapshot)
	if err != nil || decision.Request == nil || decision.Request.Name != "map.query" ||
		!strings.Contains(decision.Detail, "Refresh the Khan cooldown") {
		t.Fatalf("capped pending cooldown refresh decision: %#v err=%v", decision, err)
	}
	...
	target.EventCampCooldownRemaining = 194
	snapshot.State.Map[0]["210:942"] = target
	setAutoKhanPolicySetting(t, &snapshot, "dailyAttackLimit", 1)
	snapshot.State.DailyAttacks = State.DailyAttackState{Count: 1, ServerThreshold: 3_500, ObservedAt: now}
	decision, err = NewAutoKhanPolicy().Evaluate(t.Context(), snapshot)
	if err != nil || decision.Request == nil || decision.Request.Name != "nomad.cooldown.minute_skip" ||
		!strings.Contains(decision.Detail, "automatic Khan attacks remain capped") {
		t.Fatalf("capped automatic cooldown maintenance decision: %#v err=%v", decision, err)
	}
	...
	if err != nil || decision.Request == nil || decision.Request.Name != "nomad.cooldown.minute_skip" ||
		...
		t.Fatalf("capped manual-chain cooldown maintenance decision: %#v err=%v", decision, err)
	}
	target.EventCampCooldownRemaining = 0
	snapshot.State.Map[0]["210:942"] = target
	decision, err = NewAutoKhanPolicy().Evaluate(t.Context(), snapshot)
	if err != nil || decision.Request != nil || decision.Status != "idle" ||
		decision.Metrics["attackLaunchLocked"] != 1 || !strings.Contains(decision.Detail, "manual chain") {
		t.Fatalf("capped manual-chain idle decision: %#v err=%v", decision, err)
	}
}

func TestAutoKhanMaxRageChainZeroIsUnlimitedAndResetsByOccurrence(t *testing.T) {
	now := time.Date(2026, 8, 29, 16, 5, 0, 0, time.UTC)
	snapshot := autoKhanPolicySnapshot(t, now, 2)
	activity := snapshot.State.EventScores.ActivityByEvent[autoKhanEventID]
	activity.KhanDefense.Launches = 99
	snapshot.State.EventScores.ActivityByEvent[autoKhanEventID] = activity

	decision, err := NewAutoKhanPolicy().Evaluate(t.Context(), snapshot)
	if err != nil || decision.Request == nil || decision.Request.Name != "khan.attack" {
		t.Fatalf("disabled rage-chain limit decision: %#v err=%v", decision, err)
	}

	setAutoKhanPolicySetting(t, &snapshot, "maxRageChain", 1)
	activity.OccurrenceEndsAt = activity.OccurrenceEndsAt.Add(-7 * 24 * time.Hour)
	snapshot.State.EventScores.ActivityByEvent[autoKhanEventID] = activity
	decision, err = NewAutoKhanPolicy().Evaluate(t.Context(), snapshot)
	if err != nil || decision.Request == nil || decision.Request.Name != "khan.attack" ||
		decision.Metrics["rageChainTaunts"] != 0 {
		t.Fatalf("prior occurrence rage count leaked into new event: %#v err=%v", decision, err)
	}
}

func TestAutoKhanRageBoosterGateCoversOnlyNewAttacks(t *testing.T) {
	now := time.Date(2026, 8, 29, 16, 10, 0, 0, time.UTC)
	snapshot := autoKhanPolicySnapshot(t, now, 2)
	setAutoKhanPolicySetting(t, &snapshot, "requireActiveRageBooster", true)

	decision, err := NewAutoKhanPolicy().Evaluate(t.Context(), snapshot)
	if err != nil || decision.Request != nil || decision.Status != "gated" ||
		!strings.Contains(decision.Detail, "boi ID 27") {
		t.Fatalf("missing rage-booster decision: %#v err=%v", decision, err)
	}
	if !containsString(NewAutoKhanPolicy().WakeDomains(), "boosters") {
		t.Fatalf("Auto Khan attack wake domains omit boosters: %v", NewAutoKhanPolicy().WakeDomains())
	}
	target := snapshot.State.Map[0]["210:942"]
	target.EventCampCooldownRemaining = 194
	snapshot.State.Map[0]["210:942"] = target
	decision, err = NewAutoKhanPolicy().Evaluate(t.Context(), snapshot)
	if err != nil || decision.Request == nil || decision.Request.Name != "nomad.cooldown.minute_skip" {
		t.Fatalf("rage booster blocked reportless cooldown maintenance before an automatic attack: %#v err=%v", decision, err)
	}
	setAutoKhanPolicySetting(t, &snapshot, "attackLaunchesEnabled", false)
	decision, err = NewAutoKhanPolicy().Evaluate(t.Context(), snapshot)
	if err != nil || decision.Request == nil || decision.Request.Name != "nomad.cooldown.minute_skip" {
		t.Fatalf("rage booster blocked manual-chain cooldown maintenance: %#v err=%v", decision, err)
	}
	setAutoKhanPolicySetting(t, &snapshot, "attackLaunchesEnabled", true)
	...
	rageDecision, err := NewAutoKhanRagePolicy().Evaluate(t.Context(), snapshot)
	if err != nil || rageDecision.Request == nil || rageDecision.Request.Name != "khan.taunt" {
		t.Fatalf("missing attack booster blocked rage lane: %#v err=%v", rageDecision, err)
	}

	snapshot.State.Market.BoostersObservedAt = now
	snapshot.State.Market.Boosters[GameData.KhanRagePointsBoosterID] = State.MarketBoosterState{
		ID: GameData.KhanRagePointsBoosterID, BonusPercent: 100, ExpiresAt: now.Add(time.Hour),
	}
	target.EventCampCooldownRemaining = 0
	snapshot.State.Map[0]["210:942"] = target
	decision, err = NewAutoKhanPolicy().Evaluate(t.Context(), snapshot)
	if err != nil || decision.Request == nil || decision.Request.Name != "khan.attack" ||
		decision.Metrics["rageBoosterActive"] != 1 {
		t.Fatalf("active rage booster did not permit attack: %#v err=%v", decision, err)
	}
}
...
func TestAutoKhanRagePolicyTreatsLowerNewEventTotalAsUntriggered(t *testing.T) {
	now := time.Date(2026, 8, 29, 15, 9, 22, 0, time.UTC)
	snapshot := autoKhanPolicySnapshot(t, now, 2)
	snapshot.State.Khan.RageCampID = 1147
	snapshot.State.Khan.PlayerRage = 1740
	snapshot.State.Khan.PlayerRageCap = 1740
	snapshot.State.Khan.PlayerTotalRage = 10_080
	snapshot.State.Khan.RageObservedAt = now
	snapshot.State.Khan.LastTauntTriggeredRage = 682_930
	snapshot.State.Khan.LastTauntTriggeredAt = now.Add(-7 * 24 * time.Hour)

	decision, err := NewAutoKhanRagePolicy().Evaluate(t.Context(), snapshot)
	if err != nil || decision.Request == nil || decision.Request.Name != "khan.taunt" ||
		decision.Status != "taunting" {
		t.Fatalf("new-event full-rage decision: %#v err=%v", decision, err)
	}
}
...
func autoKhanPolicySnapshot(t *testing.T, now time.Time, sourceCastleID State.CastleID) Snapshot {
	...
	gameData, err := GameData.DecodeStore([]byte(`{
		...
		"effects":[{"effectID":900,"name":"travelSpeed","effectTypeID":15,"capID":1}],
		"effectCaps":[{"capID":1,"maxTotalBonus":100}]
	...
	gameState.EventScores.ActivityByEvent[autoKhanEventID] = State.EventActivityState{
		EventID: autoKhanEventID, OccurrenceEndsAt: now.Add(7_200 * time.Second), ObservedFrom: now.Add(-time.Hour),
	}
