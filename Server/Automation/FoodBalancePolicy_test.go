...
func TestFoodBalanceKingdomShipmentDefersSkipUntilTransportTimeIsObserved(t *testing.T) {
	now := time.Date(2026, 7, 29, 13, 0, 0, 0, time.UTC)
	gameData := foodBalanceGameData(t)
	...
	if !ready || decision.Request == nil || !decision.ReevaluateOnSuccess {
		t.Fatalf("kingdom shipment = %#v ready=%t", decision, ready)
	}
	var request map[string]json.RawMessage
	if err := json.Unmarshal(decision.Request.Arguments, &request); err != nil {
		t.Fatal(err)
	}
	for _, field := range []string{"timeSkipId", "minimumRemaining", "settleAfterTimeSkip"} {
		if _, exists := request[field]; exists {
			t.Fatalf("launch request guessed transport duration through %q: %s", field, decision.Request.Arguments)
		}
	}
	...
	var travellingRequest map[string]json.RawMessage
	if err := json.Unmarshal(decision.Request.Arguments, &travellingRequest); err != nil {
		t.Fatal(err)
	}
	if _, exists := travellingRequest["timeSkipId"]; exists {
		t.Fatalf("partial time skip was embedded before remaining time was observed: %s", decision.Request.Arguments)
	}
}
