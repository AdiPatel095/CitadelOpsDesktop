...
type autoEventBuildProfile struct {
	...
	AttackLootOnly            bool
	EventID                   int64
	IgnoreDemolitionCandidate func(GameData.BuildingDefinition) bool
}
...
func (profile autoEventBuildProfile) ignoresDemolitionCandidate(definition GameData.BuildingDefinition) bool {
	return profile.IgnoreDemolitionCandidate != nil && profile.IgnoreDemolitionCandidate(definition)
}
...
func evaluateAutoEventBuild(
	...
	if remediation := autoStormDiffRemediation(snapshot, settings, castle, catalog, normalDiff, metrics, profile); remediation != nil {
...
func autoStormDiffRemediation(
	...
	if diff.Exact {
		for _, issue := range diff.Issues {
			...
			for _, buildingID := range issue.BuildingIDs {
				if decision := autoStormBuildingRemovalDecision(snapshot.Now, settings, castle, catalog, buildingID, true, metrics, profile); decision != nil {
					return decision
				}
			}
		}
	...
	for _, issue := range diff.Issues {
		...
		for _, buildingID := range issue.BuildingIDs {
			if decision := autoStormBuildingRemovalDecision(snapshot.Now, settings, castle, catalog, buildingID, false, metrics, profile); decision != nil {
				return decision
			}
		}
	}
	if diff.Exact && len(diff.Actions) == 0 {
		for _, unmanaged := range diff.Unmanaged {
			if decision := autoStormBuildingRemovalDecision(snapshot.Now, settings, castle, catalog, unmanaged.BuildingInstanceID, true, metrics, profile); decision != nil {
				return decision
			}
		}
	}
...
func autoStormBuildingRemovalDecision(
	...
	demolitionAllowed := settings.Build.AllowDemolition &&
		...
		!profile.ignoresDemolitionCandidate(definition)
...
func evaluateAutoStormCombat(
	...
	for _, candidate := range candidates {
		...
		materializedPreset, err := AttackPresets.ResolveTroopFamilies(
			limitedPreset, autoStormTroopFamilyInventory(snapshot, settings, castle), snapshot.GameData, 1,
		)
		if err != nil {
			waitingDetail = fmt.Sprintf("Cannot resolve %s troop families: %v", preset.Name, err)
			continue
		}
...
func autoStormTroopFamilyInventory(
	snapshot Snapshot,
	settings autoStormSettings,
	castle State.CastleState,
) map[State.UnitID]int64 {
	available := make(map[State.UnitID]int64, len(castle.Units.Stationed))
	for unitID, amount := range castle.Units.Stationed {
		available[unitID] = max(int64(0), amount)
	}
	if !settings.TroopImport.Enabled {
		return available
	}
	seen := map[State.CastleID]struct{}{}
	for _, donorID := range settings.TroopImport.DonorCastleIDs {
		if _, duplicate := seen[donorID]; duplicate {
			continue
		}
		seen[donorID] = struct{}{}
		...
		for unitID, amount := range donor.Units.Stationed {
			isTool, known := autoStormUnitIsTool(snapshot.GameData, unitID)
			if !known || isTool || amount <= 0 {
				continue
			}
			available[unitID] = autoStormSaturatingAdd(available[unitID], amount)
		}
	}
	return available
}
