...
func TestAutoStormNeverDemolishesOfficiallyProtectedExtra(t *testing.T) {
	...
	decision := autoStormBuildingRemovalDecision(
		time.Now().UTC(), settings, castle, catalog, 46, true, map[string]float64{}, autoStormBuildProfile(),
	)
	if decision != nil {
...
func TestAutoStormTroopImportMaterializesFamilyFromSelectedDonorInventory(t *testing.T) {
	now := time.Date(2026, time.August, 25, 13, 30, 0, 0, time.UTC)
	state := State.NewGameState()
	...
	settings.Islands.PresetID = "family"
	settings.TroopImport.Enabled = true
	settings.TroopImport.DonorCastleIDs = []State.CastleID{donor.ID}
	presets := json.RawMessage(`{
		"version":1,
		"presets":[{"id":"family","name":"Family","useTroopFamilies":true,"waves":[{
			"L":{"troops":[],"tools":[]},
			"M":{"troops":[{"itemId":13,"quantity":5}],"tools":[]},
			"R":{"troops":[],"tools":[]}
		}]}]
	}`)
	...
	if err != nil || detail != "" || decision == nil || decision.Request == nil || decision.Request.Name != "troops.kingdom.ship" {
		t.Fatalf("family troop import decision = %#v detail=%q err=%v", decision, detail, err)
	}
	var arguments struct {
		Units []struct {
			UnitID State.UnitID `json:"unitId"`
	...
	if len(arguments.Units) != 1 || arguments.Units[0].UnitID != 14 || arguments.Units[0].Amount != 5 {
		t.Fatalf("family troop import arguments = %#v, want five tier-14 troops", arguments)
	}
}
...
func autoStormTestGameData(t *testing.T) *GameData.Store {
	...
	store, err := GameData.DecodeStore([]byte(`{
		"versionInfo":[],"buildings":[],"effects":[],"effectCaps":[],"units":[
			{"wodID":10},{"wodID":11,"slotTypes":"tool"},{"wodID":12},
			{"wodID":13,"upgradeWodID":14},{"wodID":14,"downgradeWodID":13}
		],
