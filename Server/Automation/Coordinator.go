...
// SetExternalConfigurationAuthority keeps hosted runtime progress out of the
// account-owned desired settings document. It must be set before Run starts.
func (coordinator *Coordinator) SetExternalConfigurationAuthority(enabled bool) {
	if coordinator == nil {
		return
	}
	coordinator.externalConfigurationAuthority.Store(enabled)
}
...
func (coordinator *Coordinator) evaluate(
	...
	for _, policy := range coordinator.policies {
		...
		if lock := state.Automations[policy.ID()].SafetyLock; lock.Active(now) {
			...
			coordinator.recordDecision(policy.ID(), policyEnabled(policy, enabled, state), Decision{Status: "gated", Detail: lock.Detail(), NextCheckAt: lock.ExpiresAt()}, "safety_lock")
			continue
		}
		...
		coordinator.recordDecision(policy.ID(), true, Decision{
			Status: "running", Detail: decision.Detail, NextCheckAt: decision.NextCheckAt, Metrics: decision.Metrics, Request: &request,
		})
		go func(
			policyID string,
...
func (coordinator *Coordinator) recordReceipt(result operationResult) {
	coordinator.updateAutomation(result.policyID, func(current State.AutomationState) State.AutomationState {
		...
		if operationResultAdvancesOperationalCursor(result) {
			if result.operationalCursor != nil && strings.TrimSpace(result.operationalCursor.Key) != "" {
				if current.OperationalCursors == nil {
					current.OperationalCursors = map[string]int{}
				}
				current.OperationalCursors[result.operationalCursor.Key] = result.operationalCursor.Value
			}
		}
...
func operationResultAdvancesOperationalCursor(result operationResult) bool {
	return result.receipt.Status == Intent.StatusSucceeded &&
		(result.followUp == nil || result.followUp.Status == Intent.StatusSucceeded)
}
...
func operationResultTroopAvailabilityGate(result operationResult) (troopAvailabilityGate, bool) {
	...
	gate := troopAvailabilityGate{detail: "Waiting for enough eligible troops to fill the selected attack preset"}
...
func decisionRequestFingerprint(decision Decision) string {
	...
	if decision.OperationalCursor != nil {
		parts = append(parts, "operational-cursor", decision.OperationalCursor.Key, strconv.Itoa(decision.OperationalCursor.Value))
	}
