...
func TestOwnedKingdomTransportDecisionKeepsSkipOwnershipIsolated(t *testing.T) {
	...
	if foodDecision.FailureFallback == nil || foodDecision.FailureFallback.Name != "resource.logistics.refresh" ||
		!foodDecision.ReevaluateOnStale {
		t.Fatalf("Auto Food skip did not declare state reconciliation: %#v", foodDecision)
	}
