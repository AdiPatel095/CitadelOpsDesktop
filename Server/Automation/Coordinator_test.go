...
func TestCoordinatorRecordsOperationalCursorOnlyAfterSuccessfulAction(t *testing.T) {
	...
	coordinator.recordReceipt(operationResult{
		...
		receipt:           Intent.Receipt{ID: "success", Status: Intent.StatusSucceeded},
		operationalCursor: update,
	})
	if got := state.Snapshot().Automations["autoRecruit"].OperationalCursors[update.Key]; got != update.Value {
		t.Fatalf("successful operational cursor=%d, want %d", got, update.Value)
	}

	coordinator.recordReceipt(operationResult{
		policyID:          "autoRecruit",
		receipt:           Intent.Receipt{ID: "failed", Status: Intent.StatusFailed, Error: "rejected"},
		operationalCursor: &OperationalCursorUpdate{Key: update.Key, Value: 0},
	})
	if got := state.Snapshot().Automations["autoRecruit"].OperationalCursors[update.Key]; got != update.Value {
		t.Fatalf("failed action advanced operational cursor to %d", got)
	}
}
...
func TestCoordinatorWaitsForCurrentSessionBaseline(t *testing.T) {
	...
	if automation := state.Snapshot().Automations["baseline"]; automation.Status != "waiting" ||
		automation.Detail != "Waiting for the latest game state to finish loading" {
...
func TestCoordinatorTroopShortageIsAvailabilityGateWithoutSafetyPause(t *testing.T) {
	...
	if automation.Status != "gated" ||
		automation.Detail != "Waiting for enough eligible troops to fill the selected attack preset" ||
		automation.LastError != "" || automation.NextCheckAt != nil {
...
func TestCoordinatorTroopGateParsesRawErrorAfterUserFacingLabeling(t *testing.T) {
	...
	if gate.detail != "Waiting for enough eligible troops to fill the selected attack preset" {
		t.Fatalf("visible gate detail = %q", gate.detail)
	}
}
