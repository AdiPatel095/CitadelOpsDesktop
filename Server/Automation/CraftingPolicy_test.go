...
func TestCraftingPolicyQueuesAffordableFallbackWithoutBreakingCycle(t *testing.T) {
	...
	gameState.Automations["autoSceatRes"] = State.AutomationState{
		ID:                 "autoSceatRes",
		OperationalCursors: map[string]int{craftingOperationalCursorKey("20", "1"): 1},
	}
	decision, err = NewCraftingPolicy().Evaluate(t.Context(), Snapshot{
		State: gameState,
		Configuration: Configuration.Snapshot{Sections: map[string]json.RawMessage{
		...
		GameData:                     store,
		Now:                          now,
		ConfigurationExternallyOwned: true,
	})
	if err != nil || decision.Request == nil || decision.Request.Name != "crafting.start" {
		t.Fatalf("hosted crafting decision=%+v err=%v", decision, err)
	}
	if err := json.Unmarshal(decision.Request.Arguments, &start); err != nil {
		t.Fatal(err)
	}
	if start.RecipeID != 101 {
		t.Fatalf("hosted operational cursor queued recipe %d, want 101", start.RecipeID)
	}
	if decision.FollowUp != nil {
		t.Fatalf("hosted crafting attempted a configuration follow-up: %+v", decision.FollowUp)
	}
	if decision.OperationalCursor == nil ||
		decision.OperationalCursor.Key != craftingOperationalCursorKey("20", "1") ||
		decision.OperationalCursor.Value != 0 {
		t.Fatalf("hosted crafting operational cursor=%+v", decision.OperationalCursor)
	}
}
