...
func TestBackgroundOnlyAPIRejectsBackgroundLoginWithoutWritingCredential(t *testing.T) {
	dataDir := t.TempDir()
	handler := NewServer(Config{
		BackgroundLogin: Session.NewBackgroundLoginStore(dataDir),
		BackgroundOnly:  true,
	}).Handler()
	recorder := httptest.NewRecorder()
	request := httptest.NewRequest(
		http.MethodPost,
		"/api/v2/session/background-login",
		strings.NewReader(`{"username":"private-player","password":"private-password","server":"US1"}`),
	)
	request.Header.Set("Content-Type", "application/json")
	handler.ServeHTTP(recorder, request)

	if recorder.Code != http.StatusForbidden || !strings.Contains(recorder.Body.String(), "background_login_managed") {
		t.Fatalf("hosted background login returned HTTP %d: %s", recorder.Code, recorder.Body.String())
	}
	if body := recorder.Body.String(); strings.Contains(body, "private-player") || strings.Contains(body, "private-password") {
		t.Fatalf("hosted background login rejection exposed credentials: %s", body)
	}
