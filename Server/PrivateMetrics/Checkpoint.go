...
func BuildCheckpoint(
	...
	if configuration != nil {
		if encoded, err := json.Marshal(configuration.Snapshot()); err == nil {
...
func checkpointID(placement Placement, checkpoint Checkpoint) string {
	digest := sha256.New()
	_, _ = fmt.Fprintf(
		digest, "checkpoint\x00%d\x00%s\x00%s\x00%d\x00%d\x00%s\x00%d",
		CheckpointSchemaVersion, placement.TenantID, placement.RuntimeID, placement.PlacementEpoch,
		checkpoint.StateRevision, checkpoint.Reason, checkpoint.ObservedAt.UnixNano(),
	)
...
type CheckpointPublisherConfig struct {
	...
	Client        *Client
	Placement     *Placement
	// Interval is the cadence at which a changed state revision is
	// checkpointed; it is also the base retry delay.
...
// current: on a cadence while the state changes, promptly after a session
// transition, and once more when the runtime is drained. It shares the
// runtime's private-metrics placement and follows the same retry discipline.
...
func (publisher *CheckpointPublisher) Run(ctx context.Context) {
	...
	attempt := func() {
		...
		pendingReason = CheckpointReasonCadence
		if reason == CheckpointReasonCadence && publisher.state.Revision() == publisher.Status().LastRevision && failures == 0 {
		...
		if reason == CheckpointReasonSession {
