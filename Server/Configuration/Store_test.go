...
func TestStoreAuthoritativeReplaceRemovesOmittedSectionsAtomically(t *testing.T) {
	store, err := Open(t.TempDir(), map[string]json.RawMessage{
		"scheduler": json.RawMessage(`{"delay":4}`),
		"stale":     json.RawMessage(`{"enabled":true}`),
	})
	if err != nil {
		t.Fatal(err)
	}
	...
	defer unsubscribe()
	snapshot, changed, err := store.ReplaceAllAuthoritative(map[string]json.RawMessage{
		"scheduler": json.RawMessage(`{ "delay": 9 }`),
		"new":       json.RawMessage(`{"enabled":false}`),
	})
	if err != nil {
		t.Fatal(err)
	}
	if snapshot.Revision != 1 || len(snapshot.Sections) != 2 {
		t.Fatalf("replacement snapshot = %+v", snapshot)
	}
	if _, exists := snapshot.Sections["stale"]; exists {
		t.Fatal("replacement retained an omitted section")
	}
	if strings.Join(changed, ",") != "new,scheduler,stale" {
		t.Fatalf("changed sections = %v", changed)
	}
	select {
	case event := <-events:
		if !event.Gap || event.Section != "*" || event.Revision != snapshot.Revision {
			t.Fatalf("replacement event = %+v", event)
		}
	case <-time.After(time.Second):
		t.Fatal("timed out waiting for replacement event")
	}
}

func TestStoreExternalAuthorityRejectsPortableWritesAndPreservesLocalSection(t *testing.T) {
	store, err := Open(t.TempDir(), map[string]json.RawMessage{
		"scheduler": json.RawMessage(`{"delay":4}`),
		"local":     json.RawMessage(`{"enabled":false}`),
	})
	if err != nil {
		t.Fatal(err)
	}
	store.SetExternalAuthority(true, "local")
	if _, err := store.Update("scheduler", json.RawMessage(`{"delay":8}`)); !errors.Is(err, ErrExternalAuthority) {
		t.Fatalf("portable update error = %v", err)
	}
	if _, _, err := store.UpdateMany(map[string]json.RawMessage{
		"local": json.RawMessage(`{"enabled":true}`),
		"other": json.RawMessage(`true`),
	}); !errors.Is(err, ErrExternalAuthority) {
		t.Fatalf("mixed batch update error = %v", err)
	}
	local := json.RawMessage(`{"enabled":true}`)
	if _, err := store.Update("local", local); err != nil {
		t.Fatal(err)
	}
	snapshot, _, err := store.ReplaceAllAuthoritative(map[string]json.RawMessage{
		"scheduler": json.RawMessage(`{"delay":9}`),
	}, "local")
	if err != nil {
		t.Fatal(err)
	}
	if !bytes.Equal(snapshot.Sections["local"], local) {
		t.Fatalf("local section was overwritten: %s", snapshot.Sections["local"])
	}
	if got := string(snapshot.Sections["scheduler"]); got != `{"delay":9}` {
		t.Fatalf("authoritative scheduler = %s", got)
	}
	if _, exists := snapshot.Sections["other"]; exists {
		t.Fatal("rejected batch partially mutated the store")
	}
}
