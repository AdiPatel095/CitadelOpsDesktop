...
func (transport *ChromiumTransport) Send(ctx context.Context, payload []byte) error {
	...
	allianceHelpPlayerID := transport.allianceHelpPlayerID
	allianceHelpCastleID := transport.allianceHelpCastleID
	transport.mu.RUnlock()
	...
	if chromiumContextTransitionRequest(payload) {
		transport.clearAllianceHelpContext()
	}
	if chromiumRecruitmentAHR(payload) {
		expected := metadata.ResponseIdentity
		if expected.PlayerID <= 0 || expected.CastleID <= 0 {
			return fmt.Errorf("recruitment AHR requires an expected player and castle identity")
		}
		if allianceHelpPlayerID <= 0 || allianceHelpCastleID <= 0 {
			return fmt.Errorf("recruitment AHR requires a current authoritative castle focus")
		}
		if expected.PlayerID != allianceHelpPlayerID || expected.CastleID != allianceHelpCastleID {
			return fmt.Errorf(
				"recruitment AHR focus changed before dispatch: expected player %d castle %d, observed player %d castle %d",
				expected.PlayerID, expected.CastleID, allianceHelpPlayerID, allianceHelpCastleID,
			)
		}
	}
	...
	responseIdentityLiteral, err := json.Marshal(metadata.ResponseIdentity)
	if err != nil {
		return err
	}
	...
	expression := fmt.Sprintf(
		"globalThis.__citadelSend && globalThis.__citadelSend(%s, %s, %s, %s, %s, %d, %s, %s)",
		tokenLiteral, epochLiteral, literal, responseTokenLiteral, responseOpcodesLiteral,
		metadata.ResponseTimeoutMillis, responseIdentityLiteral, causationOperationIDLiteral,
	)
	sent := false
	...
	err = chromedp.Run(evaluationContext, chromedp.ActionFunc(func(ctx context.Context) error {
		...
		if exception != nil {
			return fmt.Errorf("game websocket send: %s", exception.Error())
		}
		if result != nil {
	...
	if err != nil {
		...
		if chromiumAHRPreflightError(err) {
			return err
		}
...
func chromiumRecruitmentAHR(payload []byte) bool {
	frame, err := Protocol.Decode(string(payload), Protocol.DirectionOutbound, time.Now().UTC())
	if err != nil || frame.Opcode != "ahr" || len(frame.Payload) == 0 {
		return false
	}
	var request struct {
		Type int `json:"T"`
	}
	return json.Unmarshal(frame.Payload, &request) == nil && request.Type == 6
}

func chromiumContextTransitionRequest(payload []byte) bool {
	frame, err := Protocol.Decode(string(payload), Protocol.DirectionOutbound, time.Now().UTC())
	return err == nil && allianceHelpContextTransitionOpcode(frame.Opcode)
}

func chromiumAHRPreflightError(err error) bool {
	return err != nil && strings.Contains(err.Error(), "citadel-preflight:")
}

func (transport *ChromiumTransport) clearAllianceHelpContext() {
	transport.mu.Lock()
...
func (transport *ChromiumTransport) processSocketFrame(
	...
	if connectionGeneration, active := transport.activeSocketGeneration(generation, key); active {
		if frameErr == nil &&
			(notice.Direction == Protocol.DirectionInbound || allianceHelpContextTransitionOpcode(frame.Opcode)) {
			transport.recordAllianceHelpContext(generation, key, frame)
		}
		if notice.Direction == Protocol.DirectionInbound {
...
func (transport *ChromiumTransport) recordAllianceHelpContext(
	generation uint64,
	key chromiumSocketKey,
	frame Protocol.Frame,
) {
	clearContext := (frame.Direction == Protocol.DirectionOutbound &&
		allianceHelpContextTransitionOpcode(frame.Opcode)) ||
		(frame.Opcode == "gaa" && frame.ResponseCode != nil && *frame.ResponseCode == 0)
	playerID, castleID, focused := allianceHelpFocusIdentity(frame)
	if !clearContext && !focused {
		return
	}
	transport.mu.Lock()
...
(() => {
  const install = (root) => {
    ...
    const updateAllianceHelpContext = (record, opcode, payload) => {
      if (!record || (opcode !== 'jaa' && opcode !== 'gaa')) return;
      const parts = String(payload || '').split('%');
      const responseCode = parts.length > 4 ? Number(parts[4]) : Number.NaN;
      if (!Number.isFinite(responseCode) || responseCode !== 0) return;
      if (opcode === 'gaa') {
        record.allianceHelpPlayerId = 0;
        record.allianceHelpCastleId = 0;
        return;
      }
      const responsePayload = payloadObjectOf(payload);
      const castle = responsePayload && responsePayload.gca;
      if (!castle || typeof castle !== 'object') return;
      const address = Array.isArray(castle.A) ? castle.A : [];
      const playerId = Number(castle.O && castle.O.OID) > 0
        ? Number(castle.O.OID) : Number(address[4]);
      const castleId = Number(castle.grc && castle.grc.AID) > 0
        ? Number(castle.grc.AID) : Number(address[3]);
      if (Number.isFinite(playerId) && playerId > 0 &&
          Number.isFinite(castleId) && castleId > 0) {
        record.allianceHelpPlayerId = playerId;
        record.allianceHelpCastleId = castleId;
      }
    };
    const purgePendingResponses = (record, now) => {
      record.pendingResponses = record.pendingResponses.filter((pending) => {
        if (pending.expiresAt > now) return true;
        if (pending.requestOpcode === 'ahr') record.ahrRejectionAmbiguous = true;
        return false;
      });
    };
    const queueResponse = (record, payload, request) => {
      const opcode = opcodeOf(payload);
      if (opcode === 'gaa' || opcode === 'jaa' || opcode === 'jca') {
        record.allianceHelpPlayerId = 0;
        record.allianceHelpCastleId = 0;
      }
      purgePendingResponses(record, Date.now());
      const supplied = request && Array.isArray(request.opcodes)
        ? request.opcodes.map((value) => String(value || '').toLowerCase()).filter(Boolean)
        : [];
      const fallbackOpcodes = opcode === 'ahr' ? ['ahh', 'ahr']
        : (opcode ? [manualResponseOpcode(opcode)] : []);
      const opcodes = supplied.length > 0 ? supplied : fallbackOpcodes;
      ...
      const requestPayload = payloadObjectOf(payload);
      const responseIdentity = request && request.responseIdentity &&
        typeof request.responseIdentity === 'object' ? request.responseIdentity : {};
      if (opcode === 'ahr') {
        const activeAHR = record.pendingResponses.some((pending) => pending.requestOpcode === 'ahr');
        if (request && activeAHR) {
          throw new Error('citadel-preflight: AHR dispatch is already awaiting an authoritative response');
        }
        if (!request && activeAHR) {
          // A native/manual AHR makes any automated response wait ambiguous.
          // Drop the correlation token so its operation times out instead of
          // accepting the manual command's response as its own.
          record.ahrRejectionAmbiguous = true;
          record.pendingResponses = record.pendingResponses.filter(
            (pending) => pending.requestOpcode !== 'ahr');
        }
        if (request && requestPayload && Number(requestPayload.T) === 6) {
          const expectedPlayerId = Number(responseIdentity.playerId);
          const expectedCastleId = Number(responseIdentity.castleId);
          if (!(expectedPlayerId > 0) || !(expectedCastleId > 0) ||
              expectedPlayerId !== Number(record.allianceHelpPlayerId) ||
              expectedCastleId !== Number(record.allianceHelpCastleId)) {
            throw new Error('citadel-preflight: recruitment AHR focus changed before dispatch');
          }
        }
      }
      const pending = {
        token: request ? String(request.token || '') : '', opcodes,
        requestOpcode: opcode, requestPayload,
        requestPlayerId: Number(responseIdentity.playerId) > 0
          ? Number(responseIdentity.playerId) : record.allianceHelpPlayerId,
        requestCastleId: Number(responseIdentity.castleId) > 0
          ? Number(responseIdentity.castleId) : record.allianceHelpCastleId,
    ...
    const responseMatchesRequest = (pending, opcode, payload) => {
      ...
      if (pending && pending.requestOpcode === 'lta' && opcode === 'gam') {
        const responseParts = String(payload || '').split('%');
        const responseCode = responseParts.length > 4 ? Number(responseParts[4]) : Number.NaN;
        if (!Number.isFinite(responseCode)) return false;
        if (responseCode !== 0) return true;
        const responsePayload = payloadObjectOf(payload);
        const movements = responsePayload && Array.isArray(responsePayload.M) ? responsePayload.M : [];
        return movements.some((row) => {
          if (!row || typeof row !== 'object' || Array.isArray(row) ||
              !row.M || typeof row.M !== 'object' || Array.isArray(row.M)) return false;
          const movement = row.M;
          const source = Array.isArray(movement.SA) ? movement.SA : [];
          const target = Array.isArray(movement.TA) ? movement.TA : [];
          const unitMovement = row.UM && typeof row.UM === 'object' && !Array.isArray(row.UM)
            ? row.UM : null;
          const leader = unitMovement && unitMovement.L && typeof unitMovement.L === 'object' &&
            !Array.isArray(unitMovement.L) ? unitMovement.L : null;
          const commanderIdPresent = leader && Object.prototype.hasOwnProperty.call(leader, 'ID') &&
            leader.ID !== null && String(leader.ID).trim() !== '';
          const commanderId = commanderIdPresent ? Number(leader.ID) : Number.NaN;
          if (!(Number(movement.MID) > 0) ||
              (Number(movement.D) !== 0 && Number(movement.D) !== 1) ||
              Number(movement.T) !== 20 || Number(movement.KID) !== 0 ||
              Number(source[0]) !== 35 || Number.isFinite(commanderId) && commanderId >= 0) {
            return false;
          }
          if (Number(pending.requestCastleId) > 0 &&
              Number(target[3]) !== Number(pending.requestCastleId)) return false;
          let targetPlayerId = Number(movement.TID);
          const addressPlayerId = Number(target[4]);
          if (Number.isFinite(addressPlayerId) && addressPlayerId !== 0) targetPlayerId = addressPlayerId;
          return !(Number(pending.requestPlayerId) > 0) ||
            targetPlayerId === Number(pending.requestPlayerId);
        });
      }
      ...
      const responseParts = String(payload || '').split('%');
      const responseCode = responseParts.length > 4 ? Number(responseParts[4]) : Number.NaN;
      if (!Number.isFinite(responseCode) || responseCode !== 0) return false;
      if (!requestPayload || !responsePayload || !Number.isFinite(responsePayload.TID) ||
          responsePayload.TID !== Number(requestPayload.T)) {
        return false;
      }
      if (Number(requestPayload.T) === 2) {
        return responsePayload.OP && Number.isFinite(responsePayload.OP.RID) &&
          responsePayload.OP.RID === Number(requestPayload.ID);
      }
      if (Number(requestPayload.T) === 6) {
        return Number(pending.requestPlayerId) > 0 && Number(pending.requestCastleId) > 0 &&
          Number.isFinite(responsePayload.LID) && responsePayload.LID > 0 &&
          Number.isFinite(responsePayload.AC) && responsePayload.AC === 0 &&
          Number.isFinite(responsePayload.PID) &&
          responsePayload.PID === Number(pending.requestPlayerId) &&
          Number.isFinite(responsePayload.P) && responsePayload.P === 0 &&
          responsePayload.OP && Number.isFinite(responsePayload.OP.AID) &&
          responsePayload.OP.AID === Number(pending.requestCastleId) &&
          Number.isFinite(responsePayload.OP.RLID) && responsePayload.OP.RLID === 0;
      }
      return true;
    };
    const matchResponse = (record, opcode, payload) => {
      const now = Date.now();
      purgePendingResponses(record, now);
      let index = -1;
      const rejectionPayload = opcode === 'ahr' ? payloadObjectOf(payload) : null;
      const rejectionParts = opcode === 'ahr' && typeof payload === 'string' ? payload.split('%') : [];
      const rejectionCode = rejectionParts.length > 4 ? Number(rejectionParts[4]) : Number.NaN;
      const rejectionPayloadText = rejectionParts.length > 5 ? String(rejectionParts[5] || '').trim() : '';
      const rejectionPayloadIsEmpty = rejectionPayloadText === '' || rejectionPayloadText === 'null' ||
        (rejectionPayload && typeof rejectionPayload === 'object' && !Array.isArray(rejectionPayload) &&
          Object.keys(rejectionPayload).length === 0);
      const payloadlessRejection = opcode === 'ahr' &&
        Number.isFinite(rejectionCode) && rejectionCode !== 0 && rejectionPayloadIsEmpty;
      if (opcode === 'ahr' && !payloadlessRejection) return '';
      if (payloadlessRejection) {
        if (record.ahrRejectionAmbiguous) return '';
        const candidates = [];
        record.pendingResponses.forEach((pending, candidateIndex) => {
          if (pending.requestOpcode === 'ahr' && pending.opcodes.includes(opcode)) {
      ...
      if (index < 0 && opcode === 'gaa' && responseRows.length > 0) {
      ...
      construct(target, args) {
        ...
        if (typeof url === 'string' && url.toLowerCase().includes('ep-live')) {
          const record = {
            ...
            pendingResponses: [], allianceHelpPlayerId: 0, allianceHelpCastleId: 0,
            ahrRejectionAmbiguous: false
          };
    ...
    const send = (
      ...
      responseIdentity, causationOperationId
    ) => {
      const record = sockets.get(String(token));
  ...
  if (typeof window !== 'undefined' && window.Worker && !window.__citadelWorkerInstalled) {
    ...
    const workerBridge = (root, marker) => {
      ...
      root.addEventListener('message', (event) => {
        ...
        const respond = (result, error) => {
          postMessage({
            marker, kind: 'result', id: message.id, result: result === true,
        ...
        try {
          ...
          if (message.action === 'activate' && typeof root.__citadelActivate === 'function') {
            ...
            result = root.__citadelSend(
              ...
              message.responseIdentity, message.causationOperationId
            );
          }
          Promise.resolve(result).then(
            (value) => respond(value, ''),
            (error) => respond(false, String(error && error.message ? error.message : error || ''))
          );
        } catch (error) {
          respond(false, String(error && error.message ? error.message : error || ''));
        }
      });
    };
    ...
    const runWorkerCommand = (
      ...
      responseTimeoutMillis, responseIdentity, causationOperationId
    ) => {
      return new Promise((resolve, reject) => {
        ...
        pendingCommands.set(id, { bridge, resolve, reject, timeout });
        try {
          bridge.postMessage({
            marker: bridgeMarker, kind: 'command', id, action,
            token: String(token), epoch: String(epoch), payload, responseToken,
            responseOpcodes, responseTimeoutMillis, responseIdentity, causationOperationId
          });
        } catch (_) {
          pendingCommands.delete(id);
