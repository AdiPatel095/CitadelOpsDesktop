...
func TestDirectWebSocketRecruitmentAHRRequiresCreationForFocusedOwnCastle(t *testing.T) {
	...
	focus := decode(`%xt%jaa%1%0%{"gca":{"O":{"OID":501},"grc":{"AID":77},"A":[0,0,0,77,501]}}%`)
	if token := transport.matchResponseToken(focus); token != "" {
		t.Fatalf("castle focus unexpectedly matched response token %q", token)
	}
	metadata := Outbound.Metadata{
		ResponseToken: "recruitment-77", ResponseOpcodes: []string{"ahh", "ahr"},
		ResponseTimeoutMillis: 10_000,
		ResponseIdentity:      Outbound.ResponseIdentity{PlayerID: 501, CastleID: 77},
	}
	if pending, err := transport.registerPending(
		metadata, `%xt%EmpireEx_21%ahr%1%{"ID":0,"T":6}%`,
	); err != nil || pending == nil {
		if err != nil {
			t.Fatal(err)
		}
		t.Fatal("recruitment AHR was not registered")
	}
	...
	for _, test := range []struct {
		...
		{"other player", `%xt%ahh%1%0%{"LID":1,"PID":999,"TID":6,"P":0,"AC":0,"OP":{"AID":77,"RLID":0}}%`},
		{"other castle", `%xt%ahh%1%0%{"LID":1,"PID":501,"TID":6,"P":0,"AC":0,"OP":{"AID":88,"RLID":0}}%`},
		{"other request type", `%xt%ahh%1%0%{"LID":1,"PID":501,"TID":2,"P":0,"AC":0,"OP":{"AID":77,"RLID":0}}%`},
		{"other line", `%xt%ahh%1%0%{"LID":1,"PID":501,"TID":6,"P":0,"AC":0,"OP":{"AID":77,"RLID":1}}%`},
		{"progress one", `%xt%ahh%1%0%{"LID":1,"PID":501,"TID":6,"P":1,"AC":0,"OP":{"AID":77,"RLID":0}}%`},
		{"progress two", `%xt%ahh%1%0%{"LID":1,"PID":501,"TID":6,"P":2,"AC":0,"OP":{"AID":77,"RLID":0}}%`},
		{"progress three", `%xt%ahh%1%0%{"LID":1,"PID":501,"TID":6,"P":3,"AC":0,"OP":{"AID":77,"RLID":0}}%`},
		{"missing progress", `%xt%ahh%1%0%{"LID":1,"PID":501,"TID":6,"AC":0,"OP":{"AID":77,"RLID":0}}%`},
		{"null progress", `%xt%ahh%1%0%{"LID":1,"PID":501,"TID":6,"P":null,"AC":0,"OP":{"AID":77,"RLID":0}}%`},
		{"missing confirmation", `%xt%ahh%1%0%{"LID":1,"PID":501,"TID":6,"P":0,"OP":{"AID":77,"RLID":0}}%`},
		{"null confirmation", `%xt%ahh%1%0%{"LID":1,"PID":501,"TID":6,"P":0,"AC":null,"OP":{"AID":77,"RLID":0}}%`},
		{"missing list id", `%xt%ahh%1%0%{"PID":501,"TID":6,"P":0,"AC":0,"OP":{"AID":77,"RLID":0}}%`},
		{"missing optional payload", `%xt%ahh%1%0%{"LID":1,"PID":501,"TID":6,"P":0,"AC":0}%`},
		{"malformed optional payload", `%xt%ahh%1%0%{"LID":1,"PID":501,"TID":6,"P":0,"AC":0,"OP":[]}%`},
		{"missing line", `%xt%ahh%1%0%{"LID":1,"PID":501,"TID":6,"P":0,"AC":0,"OP":{"AID":77}}%`},
		{"null line", `%xt%ahh%1%0%{"LID":1,"PID":501,"TID":6,"P":0,"AC":0,"OP":{"AID":77,"RLID":null}}%`},
		{"error response", `%xt%ahh%1%269%{"LID":1,"PID":501,"TID":6,"P":0,"AC":0,"OP":{"AID":77,"RLID":0}}%`},
	} {
		t.Run(test.name, func(t *testing.T) {
			if token := transport.matchResponseToken(decode(test.raw)); token != "" {
				t.Fatalf("unrelated AHH consumed pending recruitment token %q", token)
			}
		})
	}
	if len(transport.pending) != 1 {
		t.Fatalf("unrelated AHH changed pending count to %d", len(transport.pending))
	}
	correct := decode(`%xt%ahh%1%0%{"LID":2,"PID":501,"TID":6,"P":0,"AC":0,"OP":{"AID":77,"RLID":0}}%`)
	if token := transport.matchResponseToken(correct); token != metadata.ResponseToken {
		t.Fatalf("matching recruitment AHH token = %q, want %q", token, metadata.ResponseToken)
	}
	if len(transport.pending) != 0 {
		t.Fatalf("matching recruitment AHH left %d pending response(s)", len(transport.pending))
	}
}

func TestDirectWebSocketKhanLTAGAMRequiresAuthoritativeRetaliation(t *testing.T) {
	transport := &DirectWebSocketTransport{}
	metadata := Outbound.Metadata{
		ResponseToken: "khan-lta", ResponseOpcodes: []string{"gam"}, ResponseTimeoutMillis: 10_000,
		ResponseIdentity: Outbound.ResponseIdentity{PlayerID: 501, CastleID: 77},
	}
	register := func(token string) {
		t.Helper()
		metadata.ResponseToken = token
		pending, err := transport.registerPending(
			metadata, `%xt%EmpireEx_21%lta%1%{"AV":0,"EID":72}%`,
		)
		if err != nil || pending == nil {
			t.Fatalf("register LTA = (%v, %v)", pending, err)
		}
	}
	decode := func(raw string) Protocol.Frame {
		t.Helper()
	...
	register("khan-lta")
	manual, err := transport.registerPending(Outbound.Metadata{
		ResponseToken: "manual-gam", ResponseOpcodes: []string{"gam"}, ResponseTimeoutMillis: 10_000,
	}, `%xt%EmpireEx_21%gam%1%{}%`)
	if err != nil || manual == nil {
		t.Fatalf("register manual GAM = (%v, %v)", manual, err)
	}
	if token := transport.matchResponseToken(decode(`%xt%gam%1%0%{"M":[]}%`)); token != "manual-gam" {
		t.Fatalf("empty GAM token = %q, want manual-gam rather than Khan LTA", token)
	}
	if len(transport.pending) != 1 || transport.pending[0].token != "khan-lta" {
		t.Fatalf("manual GAM changed Khan LTA pending state: %#v", transport.pending)
	}
	for _, test := range []struct {
		name string
		raw  string
	}{
		{name: "ordinary player movement", raw: `%xt%gam%1%0%{"M":[{"M":{"MID":10,"D":0,"TID":501,"T":2,"KID":0,"SA":[35,939,1123,0,0],"TA":[1,212,941,77,501]}}]}%`},
		{name: "wrong source type", raw: `%xt%gam%1%0%{"M":[{"M":{"MID":10,"D":0,"TID":501,"T":20,"KID":0,"SA":[2,939,1123,0,0],"TA":[1,212,941,77,501]}}]}%`},
		{name: "commander movement", raw: `%xt%gam%1%0%{"M":[{"M":{"MID":10,"D":0,"TID":501,"T":20,"KID":0,"SA":[35,939,1123,0,0],"TA":[1,212,941,77,501]},"UM":{"L":{"ID":24}}}]}%`},
		{name: "wrong target castle", raw: `%xt%gam%1%0%{"M":[{"M":{"MID":10,"D":0,"TID":501,"T":20,"KID":0,"SA":[35,939,1123,0,0],"TA":[1,212,941,88,501]}}]}%`},
		{name: "wrong target player", raw: `%xt%gam%1%0%{"M":[{"M":{"MID":10,"D":0,"TID":999,"T":20,"KID":0,"SA":[35,939,1123,0,0],"TA":[1,212,941,77,999]}}]}%`},
		{name: "missing movement id", raw: `%xt%gam%1%0%{"M":[{"M":{"D":0,"TID":501,"T":20,"KID":0,"SA":[35,939,1123,0,0],"TA":[1,212,941,77,501]}}]}%`},
	} {
		t.Run(test.name, func(t *testing.T) {
			if token := transport.matchResponseToken(decode(test.raw)); token != "" {
				t.Fatalf("unrelated GAM consumed LTA token %q", token)
			}
			if len(transport.pending) != 1 {
				t.Fatalf("unrelated GAM left %d pending response(s), want one", len(transport.pending))
			}
		})
	}
	accepted := decode(`%xt%gam%1%0%{"M":[{"M":{"MID":"10","D":"0","TID":"501","T":"20","KID":"0","SA":["35",939,1123,0,0],"TA":[1,212,941,"77","501"]},"A":[[216,751]]}]}%`)
	if token := transport.matchResponseToken(accepted); token != "khan-lta" {
		t.Fatalf("authoritative Khan GAM token = %q, want khan-lta", token)
	}
	if len(transport.pending) != 0 {
		t.Fatalf("accepted Khan GAM left %d pending response(s)", len(transport.pending))
	}

	register("khan-rejection")
	if token := transport.matchResponseToken(decode(`%xt%gam%1%273%{}%`)); token != "khan-rejection" {
		t.Fatalf("Khan GAM rejection token = %q, want khan-rejection", token)
	}
}

func TestDirectWebSocketAHRRejectionRequiresUnambiguousPendingRequest(t *testing.T) {
	...
	register := func(token string) {
		...
		pending, err := transport.registerPending(Outbound.Metadata{
			ResponseToken: token, ResponseOpcodes: []string{"ahh", "ahr"},
			ResponseTimeoutMillis: 10_000,
			ResponseIdentity:      Outbound.ResponseIdentity{PlayerID: 501, CastleID: 77},
		}, `%xt%EmpireEx_21%ahr%1%{"ID":0,"T":6}%`)
		if err != nil || pending == nil {
			if err != nil {
				t.Fatal(err)
			}
			t.Fatalf("AHR %q was not registered", token)
		}
	}
	decodeRejection := func() Protocol.Frame {
		t.Helper()
		frame, err := Protocol.Decode(`%xt%ahr%1%269%`, Protocol.DirectionInbound, time.Now().UTC())
		if err != nil {
			t.Fatal(err)
		}
		return frame
	...
	for _, test := range []struct {
		...
		{"success code", `%xt%ahr%1%0%`},
		{"identity payload", `%xt%ahr%1%269%{"ID":0,"T":6}%`},
		{"array payload", `%xt%ahr%1%269%[]%`},
		{"malformed payload", `%xt%ahr%1%269%not-json%`},
	} {
		t.Run(test.name, func(t *testing.T) {
			frame, err := Protocol.Decode(test.raw, Protocol.DirectionInbound, time.Now().UTC())
			if err != nil {
				t.Fatal(err)
			}
			if token := transport.matchResponseToken(frame); token != "" {
				t.Fatalf("non-rejection AHR consumed token %q", token)
			}
			if len(transport.pending) != 1 {
				t.Fatalf("non-rejection AHR changed pending count to %d", len(transport.pending))
			}
		})
	}
	if token := transport.matchResponseToken(decodeRejection()); token != "only-ahr" {
		t.Fatalf("unambiguous AHR rejection token = %q, want only-ahr", token)
	}
	register("first-ahr")
	if pending, err := transport.registerPending(Outbound.Metadata{
		ResponseToken: "second-ahr", ResponseOpcodes: []string{"ahh", "ahr"},
		ResponseTimeoutMillis: 10_000,
		ResponseIdentity:      Outbound.ResponseIdentity{PlayerID: 501, CastleID: 77},
	}, `%xt%EmpireEx_21%ahr%1%{"ID":0,"T":6}%`); err == nil || pending != nil {
		t.Fatalf("second active AHR = (%v, %v), want deterministic rejection", pending, err)
	}
	// An ambiguous list can still arise from legacy/manual registrations that
	// predate this guard. A payloadless rejection must not choose between them.
	transport.pending = append(transport.pending, directPendingResponse{
		token: "legacy-manual-ahr", requestOpcode: "ahr",
		opcodes: map[string]struct{}{"ahr": {}}, expiresAt: time.Now().Add(time.Second),
	})
	if token := transport.matchResponseToken(decodeRejection()); token != "" {
		t.Fatalf("ambiguous AHR rejection consumed token %q", token)
	}
	if len(transport.pending) != 2 {
		t.Fatalf("ambiguous rejection changed pending count to %d", len(transport.pending))
	}
}

func TestDirectWebSocketRecruitmentAHRFailsClosedWhenFocusIdentityChanged(t *testing.T) {
	transport := &DirectWebSocketTransport{allianceHelpPlayerID: 501, allianceHelpCastleID: 88}
	pending, err := transport.registerPending(Outbound.Metadata{
		ResponseToken: "recruitment-77", ResponseOpcodes: []string{"ahh", "ahr"},
		ResponseTimeoutMillis: 10_000,
		ResponseIdentity:      Outbound.ResponseIdentity{PlayerID: 501, CastleID: 77},
	}, `%xt%EmpireEx_21%ahr%1%{"ID":0,"T":6}%`)
	if err == nil || pending != nil || !strings.Contains(err.Error(), "focus changed before dispatch") {
		t.Fatalf("mismatched focus registration = (%v, %v), want fail-closed error", pending, err)
	}
	if len(transport.pending) != 0 {
		t.Fatalf("mismatched focus left %d pending response(s)", len(transport.pending))
	}
}

func TestDirectWebSocketRecruitmentAHRFailsClosedAsMapRequestLeavesSocket(t *testing.T) {
	transport := &DirectWebSocketTransport{}
	focus, err := Protocol.Decode(
		`%xt%jaa%1%0%{"gca":{"O":{"OID":501},"grc":{"AID":77}}}%`,
		Protocol.DirectionInbound, time.Now().UTC(),
	)
	if err != nil {
		t.Fatal(err)
	}
	transport.matchResponseToken(focus)
	transport.invalidateAllianceHelpContextForOutbound(
		`%xt%EmpireEx_21%gaa%1%{"KID":0,"AX1":0,"AY1":0,"AX2":1,"AY2":1}%`,
	)
	pending, registerErr := transport.registerPending(Outbound.Metadata{
		ResponseToken: "recruitment-77", ResponseOpcodes: []string{"ahh", "ahr"},
		ResponseTimeoutMillis: 10_000,
		ResponseIdentity:      Outbound.ResponseIdentity{PlayerID: 501, CastleID: 77},
	}, `%xt%EmpireEx_21%ahr%1%{"ID":0,"T":6}%`)
	if registerErr == nil || pending != nil ||
		!strings.Contains(registerErr.Error(), "current authoritative castle focus") {
		t.Fatalf("post-map AHR registration = (%v, %v), want fail-closed error", pending, registerErr)
	}
}

func TestDirectWebSocketInvalidatesAHRContextForEveryOutboundTransition(t *testing.T) {
	for _, payload := range []string{
		`%xt%EmpireEx_21%gaa%1%{"KID":0,"AX1":0,"AY1":0,"AX2":1,"AY2":1}%`,
		`%xt%EmpireEx_21%jaa%1%{"PX":10,"PY":11,"KID":0}%`,
		`%xt%EmpireEx_21%jca%1%{"CID":77,"KID":0}%`,
	} {
		transport := &DirectWebSocketTransport{allianceHelpPlayerID: 501, allianceHelpCastleID: 77}
		...
		if transport.allianceHelpPlayerID != 0 || transport.allianceHelpCastleID != 0 {
			t.Fatalf("context transition %q retained player %d castle %d", payload,
...
func TestDirectWebSocketLateAHRRejectionDoesNotMatchNewerRequest(t *testing.T) {
	transport := &DirectWebSocketTransport{allianceHelpPlayerID: 501, allianceHelpCastleID: 77}
	pending, err := transport.registerPending(Outbound.Metadata{
		ResponseToken: "expired-ahr", ResponseOpcodes: []string{"ahh", "ahr"},
		ResponseTimeoutMillis: 1,
		ResponseIdentity:      Outbound.ResponseIdentity{PlayerID: 501, CastleID: 77},
	}, `%xt%EmpireEx_21%ahr%1%{"ID":0,"T":6}%`)
	if err != nil || pending == nil {
		t.Fatalf("register expiring AHR = (%v, %v)", pending, err)
	}
	transport.pending[0].expiresAt = time.Now().Add(-time.Second)
	newer, newerErr := transport.registerPending(Outbound.Metadata{
		ResponseToken: "newer-ahr", ResponseOpcodes: []string{"ahh", "ahr"},
		ResponseTimeoutMillis: 10_000,
		ResponseIdentity:      Outbound.ResponseIdentity{PlayerID: 501, CastleID: 77},
	}, `%xt%EmpireEx_21%ahr%1%{"ID":0,"T":6}%`)
	if newerErr != nil || newer == nil {
		t.Fatalf("register newer AHR = (%v, %v)", newer, newerErr)
	}
	rejection, decodeErr := Protocol.Decode(
		`%xt%ahr%1%269%`, Protocol.DirectionInbound, time.Now().UTC(),
	)
	if decodeErr != nil {
		t.Fatal(decodeErr)
	}
	if token := transport.matchResponseToken(rejection); token != "" {
		t.Fatalf("late rejection matched newer token %q", token)
	}
	if len(transport.pending) != 1 {
		t.Fatalf("late rejection left %d pending response(s), want newer request", len(transport.pending))
	}
	created, decodeErr := Protocol.Decode(
		`%xt%ahh%1%0%{"LID":2,"PID":501,"TID":6,"P":0,"AC":0,"OP":{"AID":77,"RLID":0}}%`,
		Protocol.DirectionInbound, time.Now().UTC(),
	)
	if decodeErr != nil {
		t.Fatal(decodeErr)
	}
	if token := transport.matchResponseToken(created); token != "newer-ahr" {
		t.Fatalf("exact newer AHH token = %q, want newer-ahr", token)
	}
}
