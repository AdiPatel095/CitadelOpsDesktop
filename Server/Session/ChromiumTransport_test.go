...
func TestChromiumRecruitmentAHRFailsClosedOnOutboundMapTransition(t *testing.T) {
	transport := newSocketTestTransport()
	processSocketTestNotice(t, transport, 1, createdSocketNotice("active", 1))
	processSocketTestNotice(t, transport, 1, socketFrameNotice(
		"active", 2, "inbound", `%xt%lli%1%0%{}%`,
	))
	<-transport.frames
	processSocketTestNotice(t, transport, 1, socketFrameNotice(
	...
	<-transport.frames
	if transport.allianceHelpPlayerID != 501 || transport.allianceHelpCastleID != 77 {
		t.Fatalf(
			"JAA focus identity = player %d castle %d, want player 501 castle 77",
			transport.allianceHelpPlayerID, transport.allianceHelpCastleID,
		)
	}
	mismatchContext := Outbound.WithMetadata(context.Background(), Outbound.Metadata{
		ConnectionGeneration: transport.status.ConnectionGeneration,
		ResponseToken:        "recruitment-88", ResponseOpcodes: []string{"ahh", "ahr"},
		ResponseTimeoutMillis: 10_000,
		ResponseIdentity:      Outbound.ResponseIdentity{PlayerID: 501, CastleID: 88},
	})
	if mismatchErr := transport.Send(
		mismatchContext, []byte(`%xt%EmpireEx_21%ahr%1%{"ID":0,"T":6}%`),
	); mismatchErr == nil || !strings.Contains(mismatchErr.Error(), "focus changed before dispatch") {
		t.Fatalf("mismatched recruitment AHR error = %v, want fail-closed focus error", mismatchErr)
	}
	processSocketTestNotice(t, transport, 1, socketFrameNotice(
		"active", 4, "outbound",
	...
	if transport.allianceHelpPlayerID != 0 || transport.allianceHelpCastleID != 0 {
		t.Fatalf(
			"GAA map transition retained player %d castle %d",
			transport.allianceHelpPlayerID, transport.allianceHelpCastleID,
		)
	}
	...
	err := transport.Send(ctx, []byte(`%xt%EmpireEx_21%ahr%1%{"ID":0,"T":6}%`))
	if err == nil || !strings.Contains(err.Error(), "current authoritative castle focus") {
		t.Fatalf("post-map recruitment AHR error = %v, want fail-closed focus error", err)
	}
}

func TestChromiumAHRPreflightErrorsAreDeterministic(t *testing.T) {
	guard := errors.New("game websocket send: Error: citadel-preflight: AHR already pending")
	if !chromiumAHRPreflightError(guard) {
		t.Fatal("known pre-send AHR guard was not classified as deterministic")
	}
	if chromiumAHRPreflightError(errors.New("game websocket send: connection closed")) {
		t.Fatal("transport failure was classified as a deterministic AHR guard")
	}
}

func TestChromiumRecognizesEveryAHRContextTransitionRequest(t *testing.T) {
	for _, payload := range []string{
		`%xt%EmpireEx_21%gaa%1%{"KID":0,"AX1":0,"AY1":0,"AX2":1,"AY2":1}%`,
		`%xt%EmpireEx_21%jaa%1%{"PX":10,"PY":11,"KID":0}%`,
		`%xt%EmpireEx_21%jca%1%{"CID":77,"KID":0}%`,
	} {
		if !chromiumContextTransitionRequest([]byte(payload)) {
			t.Fatalf("context transition was not recognized: %s", payload)
		}
	}
	if chromiumContextTransitionRequest([]byte(`%xt%EmpireEx_21%ahr%1%{"ID":0,"T":6}%`)) {
		t.Fatal("recruitment AHR was classified as a context transition")
	}
}
...
func TestChromiumTransportInjectionBridgesDedicatedWorkerSockets(t *testing.T) {
	required := []string{
		...
		"pending.requestOpcode === 'lta' && opcode === 'gam'",
		"const movements = responsePayload && Array.isArray(responsePayload.M)",
		"return movements.some((row) =>",
		"Number(movement.T) !== 20 || Number(movement.KID) !== 0",
		"Number(source[0]) !== 35",
		"Number.isFinite(commanderId) && commanderId >= 0",
		"Number(target[3]) !== Number(pending.requestCastleId)",
		...
		"opcode === 'gaa' || opcode === 'jaa' || opcode === 'jca'",
		"request.responseIdentity",
		"AHR dispatch is already awaiting an authoritative response",
		"citadel-preflight: recruitment AHR focus changed before dispatch",
		...
		"Number.isFinite(rejectionCode) && rejectionCode !== 0",
