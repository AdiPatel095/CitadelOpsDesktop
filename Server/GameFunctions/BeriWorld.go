...
func processBeriWorld() {
	...
	if gs.BeriWorldCastle.Aid == 0 {
		return
	}

	// 1. Fetch Fresh Data (Perform JAA)
	log.Printf("[BeriWorld] Fetching fresh data for Beri Castle (AID %.0f)", gs.BeriWorldCastle.Aid)
	// FetchCastleTroops uses JAA and populates buildings now
	troops := GameParser.FetchCastleTroops(10, int(gs.BeriWorldCastle.Aid), int(gs.BeriWorldCastle.Troops.X), int(gs.BeriWorldCastle.Troops.Y))
	if troops == nil {
		log.Println("[BeriWorld] Failed to fetch castle data via JAA.")
		return
	}

	castle := gs.GetCastleByID(int(gs.BeriWorldCastle.Aid))
	if castle == nil {
		return
	}
...
func moveBuildingsToIdeal(castle *Models.PlayerCastleInfo) bool {
	for i := range castle.Buildings {
		b := &castle.Buildings[i]
		...
		if !isCurrentIdeal {
			...
			for _, ip := range idealsForType {
				occupied := false
				for _, otherB := range castle.Buildings {
...
func performBuildCommands(castle *Models.PlayerCastleInfo) {
	...
	for wid, ideals := range IdealBeriMap {
		for _, ip := range ideals {
			...
			alreadyBuilt := false
			for _, b := range castle.Buildings {
