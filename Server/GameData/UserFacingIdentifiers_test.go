...
func TestIdentifierLabelsHumanizeKnownRuntimeAndOfficialIDs(t *testing.T) {
	...
	for _, want := range []string{
		"Main Keep (castle ID 10)",
		"The Bold (commander ID 20)",
		"Castellan of Main Keep (castellan ID 30)",
		"Relic Armor (equipment ID 40)",
		"Fire Gem level 7 (gem ID 50)",
		"Bakery level 8 (building instance ID 1001)",
		"Bakery level 8 (building definition ID 201)",
		"Bakery Production Item level 3 (construction item ID 301)",
		"Honey level 2 (crafting recipe ID 401)",
		"Veteran Crossbowman (unit ID 489)",
		"Mead Mace (tool ID 614)",
		"Coins (resource ID 1)",
		"Rubies (currency ID 2)",
		"War Horn Bundle (package ID 701)",
		"Fast Horse (horse ID 801)",
		"castle 999",
		"Great Empire (kingdom ID 0)",
		"Master Builder (Hall of Legends skill ID 901)",
		"Berimond Vanguard (camp ID 902)",
		"Architect (achievement ID 903)",
		"Honey Reward (reward ID 904)",
		"Travel Speed (effect ID 905)",
		"Castle Lord (player ID 60)",
		"The Citadel (alliance ID 70)",
		"Movement from Main Keep to 44:55 (movement ID 80)",
		"War of the Realms (event ID 103)",
		"Expert Plus (difficulty ID 1001)",
		"Small Wood island level 70 (Storm isle ID 906)",
		"Auto Bird target Main Keep (castle ID 10)",
		"gem carrier Relic Armor (equipment ID 40)",
	} {
...
func TestIdentifierLabelsDoNotGuessUnknownOrBareNumbers(t *testing.T) {
	labels := NewIdentifierLabels(State.NewGameState(), nil, nil)
	message := "attack 12:34 has 50 troops; castle 999 and item 888 are unavailable"
	if got := labels.Humanize(message); got != message {
		t.Fatalf("unknown identifiers changed: %q", got)
	}
}
