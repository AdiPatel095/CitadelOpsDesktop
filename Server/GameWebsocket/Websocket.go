...
func handleCDPEvent(ev interface{}) {
	...
	case *network.EventWebSocketFrameReceived:
		if gameRequestIDs[string(ev.RequestID)] {
			...
			logGameMessage("RECV", payload)
			messageParts := strings.Split(payload, "%")
			go func() {
				Channels.IncomingMessages <- messageParts
			}()
		}
	}
}

func logGameMessage(direction string, payload string) {
	timestamp := time.Now().Format("2006/01/02 15:04:05.000000")
	label := "UNKNOWN"

	if strings.HasPrefix(payload, "%xt%") {
		parts := strings.Split(payload, "%")
		if len(parts) > 2 {
			label = parts[2]
		}
	} else if strings.HasPrefix(payload, "<msg") {
		// Extract action='...'
		if idx := strings.Index(payload, "action='"); idx != -1 {
			start := idx + 8
			if end := strings.Index(payload[start:], "'"); end != -1 {
				label = payload[start : start+end]
			}
		} else if idx := strings.Index(payload, "action=\""); idx != -1 {
			start := idx + 8
			if end := strings.Index(payload[start:], "\""); end != -1 {
				label = payload[start : start+end]
			}
	...
	logLine := fmt.Sprintf("%s [%s] [%s] %s\n", timestamp, direction, label, payload)

	f, err := os.OpenFile("game_websocket.log", os.O_APPEND|os.O_CREATE|os.O_WRONLY, 0644)
	if err != nil {
		return
	}
	defer f.Close()
...
// WipeLog clears the game_websocket.log file
func WipeLog() {
	os.Remove("game_websocket.log")
}
...
func StartWebsocketChannels(ctx context.Context) {
	...
	go func() {
		for {
			...
			case <-ctx.Done():
				return
			case message := <-Channels.OutgoingMessages:
				var payload []byte
				cost := 0

				switch v := message.(type) {
				case Channels.OutgoingMessageWithCost:
					payload = v.Payload
					cost = v.Cost
				case []byte:
					payload = v
...
func incomingMessageParserStartup() {
	for message := range Channels.IncomingMessages {
		if len(message) > 3 {
			if message[2] == "lli" {
