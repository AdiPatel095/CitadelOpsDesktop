...
func runAutoBird(ctx context.Context) {
	...
	for {
		...
		if len(playerCastles) > 0 {
			...
			playerOID = int(gs.Castle.MainCastle.Aid)
		}
		...
		minDelay := delayCfg.MinDelay
		maxDelay := delayCfg.MaxDelay
		...
		birdIgnore := Models.GetSettingsState().BirdIgnoreList
		for _, castleLoc := range playerCastles {
			...
			for id, amount := range troops.Troops {
				saveAmountConfigured, configured := birdIgnore.GetSaveAmount(castleLoc.CastleID, id)
				actualSaveAmount := 0
				sendAmount := 0
...
func sendTroopRequest(kingdomID, castleID, x, y int) *Models.CastleTroops {
	...
	Channels.OutgoingMessages <- []byte(GameCommands.TroopFocusCommand(kingdomID, castleID, x, y))
...
// parseTroopsFromJAA delegates to GameParser troop + building parsers (same as CastleTroopsFetcher).
func parseTroopsFromJAA(data string, kingdomID, castleID, x, y int) *Models.CastleTroops {
	troops := GameParser.ParseCastleTroops(data, kingdomID, x, y)
	if troops == nil {
		return nil
	}
	troops.CastleID = castleID
	troops.BGRows = GameParser.ParseBGFromJAAResponseJSON(data)
	troops.BDRows = GameParser.ParseBDFromJAAResponseJSON(data)
	return troops
}
...
func reconcileOnStartup(ctx context.Context) (time.Duration, bool) {
	...
	log.Printf("[AutoBird] Reconciling %d birds against %d active movements", len(stillPotentiallyActive), len(gs.Movement.ActiveMovements))
