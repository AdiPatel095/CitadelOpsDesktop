...
func reduceCastleSnapshot(
	...
	if !frame.ReceivedAt.IsZero() {
		// JCA refreshes resolve as a JAA snapshot. Expansion/building responses
		// can also carry gca and reuse this reducer, but they do not carry the
		// embedded production lines required by the context invariant.
		if strings.EqualFold(frame.Opcode, "jaa") {
			castle.ContextSnapshotObservedAt = frame.ReceivedAt.UTC()
		}
