...
func reconcileKhanTaunts(
	...
	for _, movement := range movements {
		...
		if !exists {
			...
			occurrence, _ := gameState.LookupEventOccurrence(72)
			if !gameState.Khan.TauntCursorIncludes(gameState.Khan.PlayerTotalRage, occurrence) {
				...
				gameState.Khan.LastTauntTriggeredEventEndsAt = occurrence.EndsAt
			}
