...
func TestCastleSnapshotKeepsConstructionSlots(t *testing.T) {
	...
	if !castle.ContextSnapshotObservedAt.Equal(observedAt) {
		t.Fatalf("castle-context observation = %v, want %v", castle.ContextSnapshotObservedAt, observedAt)
	}
...
func TestCastleDetailsRefreshUpdatesExplicitDonorWithoutChangingFocus(t *testing.T) {
	now := time.Date(2026, 8, 26, 23, 0, 0, 0, time.UTC)
	contextObservedAt := now.Add(-time.Hour)
	gameState := State.NewGameState()
	gameState.Castles[100] = State.CastleState{
		ID: 100, KingdomID: 0, ContextSnapshotObservedAt: contextObservedAt,
		Units: State.CastleUnits{Stationed: map[State.UnitID]int64{10: 1}},
	}
	gameState.Castles[900] = State.CastleState{
		ID: 900, KingdomID: 10, Focused: true,
		Units: State.CastleUnits{Stationed: map[State.UnitID]int64{20: 5}},
	}
	code := 0
	domains, changed, err := reduceCastleDetails(t.Context(), Protocol.Frame{
		Direction: Protocol.DirectionInbound, Opcode: "dcl", ResponseCode: &code, ReceivedAt: now,
		Payload: json.RawMessage(`{
			"C":[
				{"KID":0,"AI":[{"AID":100,"AC":[[10,50]],"TU":[[10,3]],"HI":[],"SHI":[]}]},
				{"KID":10,"AI":[{"AID":900,"AC":[[20,7]],"TU":[],"HI":[],"SHI":[]}]}
			]
		}`),
	}, &gameState, nil)
	if err != nil || !changed || !slices.Contains(domains, "units") {
		t.Fatalf("DCL donor refresh: changed=%t domains=%v err=%v", changed, domains, err)
	}
	donor := gameState.Castles[100]
	if donor.Units.Stationed[10] != 50 || donor.Units.Traveling[10] != 3 ||
		!donor.UnitsObservedAt.Equal(now) {
		t.Fatalf("DCL did not authoritatively refresh explicit donor inventory: %#v", donor)
	}
	if donor.Focused || !gameState.Castles[900].Focused {
		t.Fatalf("DCL changed castle focus: donor=%t target=%t", donor.Focused, gameState.Castles[900].Focused)
	}
