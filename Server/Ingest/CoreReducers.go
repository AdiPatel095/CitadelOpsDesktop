...
func RegisterCoreReducers(registry *Registry) error {
	...
	sequenceGroups := []struct {
		...
		{[]string{"jaa"}, []reducerStep{
			...
			{writes: castles, reducer: reduceEmbeddedProductionSnapshots},
		...
		{[]string{"hru", "hdu"}, []reducerStep{
			...
			{writes: castles, reducer: reduceProductionSnapshot},
			{writes: resources, reducer: reduceResponseResources},
		}},
	...
	if err := registry.RegisterOutboundComponents(
		"ahr", components(State.ComponentAllianceHelp, State.ComponentCastles), reduceAllianceHelpCommand,
	); err != nil {
		return err
	}
