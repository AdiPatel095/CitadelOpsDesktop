...
func TestExpansionResponseReconcilesNestedCastleAndResourcesWithoutRootKingdom(t *testing.T) {
	...
	castle.KingdomID = 4
	castle.Focused = true
	contextObservedAt := time.Now().UTC().Add(-time.Hour)
	castle.ContextSnapshotObservedAt = contextObservedAt
	...
	if !castle.ContextSnapshotObservedAt.Equal(contextObservedAt) {
		t.Fatalf("expansion response changed JAA context timestamp: %v", castle.ContextSnapshotObservedAt)
	}
