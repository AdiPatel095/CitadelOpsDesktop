...
const tenantSessionCookie = "citadelops_tenant_session"
...
func NewDynamicTenantAuthenticator(sessionKey []byte, secureCookies bool) (*TenantAuthenticator, error) {
	...
	return &TenantAuthenticator{
		credentials: map[AccountID]tenantCredential{}, sessionKey: key, secureCookies: secureCookies,
		sessionTTL: defaultTenantSessionHours * time.Hour,
	}, nil
}
...
func (auth *TenantAuthenticator) RegisterRuntime(id AccountID) error {
	...
	auth.credentials[id] = tenantCredential{generation: 1}
	return nil
}

// SetDashboardGrant replaces the runtime's connection token without retaining
// the secret. A changed token increments the session generation, immediately
// invalidating cookies issued from the previous grant.
...
// dashboard access. Callers pass a digest so the raw token never crosses the
...
func (auth *TenantAuthenticator) newSession(id AccountID, now time.Time) string {
	auth.mu.RLock()
	credential, exists := auth.credentials[id]
	auth.mu.RUnlock()
	if !exists {
	...
	payload := "v2." + string(id) + "." + strconv.FormatUint(credential.generation, 10) + "." + strconv.FormatInt(now.Add(auth.sessionTTL).Unix(), 10)
	return payload + "." + base64.RawURLEncoding.EncodeToString(auth.sign(payload))
}
...
func (auth *TenantAuthenticator) login(writer http.ResponseWriter, request *http.Request) {
	...
	authenticatedID, valid := auth.authenticateToken(token)
	if parseErr != nil || !valid || authenticatedID != id {
	...
	session := auth.newSession(id, time.Now())
	if session == "" {
		writeRouterError(writer, http.StatusUnauthorized, "invalid_credentials")
		return
	}
	...
	_ = json.NewEncoder(writer).Encode(map[string]string{"accountId": string(id), "path": path})
}
