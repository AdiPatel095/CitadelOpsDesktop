...
func TestDashboardBootstrapCreatesOneShortTenantSession(t *testing.T) {
	supervisor, dashboardAuth, orchestrator, now := newTestOrchestrator(t)
	alpha := testAssignment("alpha", "tenant-one", 4, now.Add(10*time.Minute))
	if _, err := orchestrator.Reconcile(t.Context(), ReconcileRequest{
	...
	response, body := controlRequest(t, http.MethodPut,
		controlServer.URL+"/orchestrator/v1/runtimes/alpha/dashboard-bootstrap",
		DashboardBootstrapRequest{PlacementEpoch: 4, Token: bootstrap, ExpiresAt: now.Add(time.Minute)})
	if response.StatusCode != http.StatusCreated || bytes.Contains(body, []byte(bootstrap)) {
		t.Fatalf("install bootstrap = %d %s", response.StatusCode, body)
	}

	runtimeServer := httptest.NewServer(supervisor.Handler(dashboardAuth, nil))
	defer runtimeServer.Close()
	_ = getRuntimeState(t, runtimeServer.URL+"/accounts/alpha/api/v2/state", bootstrap, http.StatusUnauthorized)
	...
	login := func() *httptest.ResponseRecorder {
		...
		dashboardAuth.LoginHandler().ServeHTTP(result, request)
		return result
	}
	first := login()
	if first.Code != http.StatusOK || len(first.Result().Cookies()) != 1 ||
		!strings.Contains(first.Body.String(), `"expiresAt"`) {
		t.Fatalf("bootstrap login = %d %s", first.Code, first.Body.String())
	}
	...
	authenticated.AddCookie(first.Result().Cookies()[0])
	if id, valid := dashboardAuth.Authenticate(authenticated); !valid || id != "alpha" {
		t.Fatalf("bootstrap tenant session = %q %t", id, valid)
	}

	response, _ = controlRequest(t, http.MethodPut,
		controlServer.URL+"/orchestrator/v1/runtimes/alpha/dashboard-bootstrap",
		DashboardBootstrapRequest{PlacementEpoch: 3, Token: strings.Repeat("y", 48), ExpiresAt: now.Add(time.Minute)})
	if response.StatusCode != http.StatusConflict {
		t.Fatalf("stale placement bootstrap = %d", response.StatusCode)
	}
	response, _ = controlRequest(t, http.MethodPut,
		controlServer.URL+"/orchestrator/v1/runtimes/alpha/dashboard-bootstrap",
		DashboardBootstrapRequest{PlacementEpoch: 4, Token: strings.Repeat("z", 48), ExpiresAt: now.Add(3 * time.Minute)})
	if response.StatusCode != http.StatusBadRequest {
		t.Fatalf("long bootstrap = %d", response.StatusCode)
	}
}
...
func TestOrchestratorStreamsSanitizedLiveCellStatus(t *testing.T) {
	...
	if event != "orchestrator.ready" || status.CellID != "cell-one" || status.DesiredRevision != 1 || len(status.Runtimes) != 1 ||
		status.Version != App.Version || status.BuildRevision != App.BuildRevision || status.BuildID != App.BuildID {
...
func TestOrchestratorFencesAndAcknowledgesAccountConfiguration(t *testing.T) {
	supervisor, _, orchestrator, now := newTestOrchestrator(t)
	snapshot := Configuration.Snapshot{
		...
		Sections: map[string]json.RawMessage{
			"scheduler":           json.RawMessage(`{"minAttackDelay":9}`),
			"automation.autoKhan": json.RawMessage(`{"version":1,"maxRageChain":3}`),
		},
	}
	...
	alpha.DesiredConfigurationRevision = snapshot.Revision
	alpha.DesiredConfigurationDigest = digest
	if _, err := orchestrator.Reconcile(t.Context(), ReconcileRequest{
		SchemaVersion: 1, Revision: 1, Runtimes: []RuntimeAssignment{alpha},
	}); err != nil {
		t.Fatal(err)
	...
	if status.ConfigurationState != "pending" || status.AppliedConfigurationRevision != 0 {
		t.Fatalf("status before sync = %+v", status)
	}

	controlServer := httptest.NewServer(orchestrator.Handler())
	defer controlServer.Close()
	configurationURL := controlServer.URL + "/orchestrator/v1/runtimes/alpha/configuration"
	if response, _ := controlRequest(t, http.MethodPut,
		controlServer.URL+"/orchestrator/v1/runtimes/alpha/login",
		LoginCredentialRequest{PlacementEpoch: alpha.PlacementEpoch, Username: "alpha", Password: "secret", Server: "US1"},
	); response.StatusCode != http.StatusConflict {
		t.Fatalf("login before configuration sync = %d", response.StatusCode)
	}
	request := ConfigurationSyncRequest{
		SchemaVersion: 1, PlacementEpoch: alpha.PlacementEpoch, Digest: digest, Configuration: snapshot,
	}
	stale := request
	stale.PlacementEpoch--
	if response, _ := controlRequest(t, http.MethodPut, configurationURL, stale); response.StatusCode != http.StatusConflict {
		t.Fatalf("stale placement sync = %d", response.StatusCode)
	}
	badDigest := request
	badDigest.Digest = strings.Repeat("0", sha256.Size*2)
	if response, _ := controlRequest(t, http.MethodPut, configurationURL, badDigest); response.StatusCode != http.StatusBadRequest {
		t.Fatalf("bad digest sync = %d", response.StatusCode)
	}
	response, body := controlRequest(t, http.MethodPut, configurationURL, request)
	if response.StatusCode != http.StatusOK || !bytes.Contains(body, []byte(`"configurationState":"ready"`)) {
		t.Fatalf("configuration sync = %d %s", response.StatusCode, body)
	}
	if response, body = controlRequest(t, http.MethodPut, configurationURL, request); response.StatusCode != http.StatusOK {
		t.Fatalf("idempotent configuration sync = %d %s", response.StatusCode, body)
	}
	stored, ok := application.Configuration.Section("scheduler")
	if !ok || !bytes.Equal(stored, json.RawMessage(`{"minAttackDelay":9}`)) {
		t.Fatalf("runtime scheduler configuration = %s", stored)
	}
	if _, exists := application.Configuration.Section("legacy.runtime-only"); exists {
		t.Fatal("exact account configuration retained an omitted runtime-only section")
	}
	connection, _ := application.Configuration.Section("session.connection")
	...
	if !bytes.Equal(retention, localRetention) {
		t.Fatalf("installation-local retention changed during account sync: %s", retention)
	}
	status = orchestrator.Status().Runtimes[0]
	...
	withConsent.Revision++
	withConsent.Sections = map[string]json.RawMessage{
		Reports.BattleResearchConfigurationSection: json.RawMessage(`{"enabled":true}`),
		History.PlayerSamplesConfigurationSection:  json.RawMessage(`{"version":1,"retention":"unlimited"}`),
	}
	_, consentDigest, err := canonicalConfigurationDigest(withConsent)
	if err != nil {
		t.Fatal(err)
	}
	alpha.DesiredConfigurationRevision = withConsent.Revision
	alpha.DesiredConfigurationDigest = consentDigest
	if _, err := orchestrator.Reconcile(t.Context(), ReconcileRequest{
		SchemaVersion: 1, Revision: 2, Runtimes: []RuntimeAssignment{alpha},
	}); err != nil {
		t.Fatal(err)
	}
	if session := application.State.Session().Status; session == "stopped" {
		t.Fatal("configuration update disconnected an existing session")
	}
	response, _ = controlRequest(t, http.MethodPut, configurationURL, ConfigurationSyncRequest{
		SchemaVersion: 1, PlacementEpoch: alpha.PlacementEpoch, Digest: consentDigest, Configuration: withConsent,
	})
	if response.StatusCode != http.StatusUnprocessableEntity {
		t.Fatalf("installation-scoped configuration sync = %d", response.StatusCode)
	}
}

func TestOrchestratorReappliesAcknowledgedConfigurationAfterApplicationReplacement(t *testing.T) {
	supervisor, _, orchestrator, now := newTestOrchestrator(t)
	snapshot := Configuration.Snapshot{
		SchemaVersion: Configuration.SchemaVersion,
		Revision:      4,
		UpdatedAt:     now,
		Sections: map[string]json.RawMessage{
			"scheduler": json.RawMessage(`{"timezone":"UTC"}`),
		},
	}
	_, digest, err := canonicalConfigurationDigest(snapshot)
	if err != nil {
	...
	assignment := testAssignment("alpha", "tenant-one", 3, now.Add(10*time.Minute))
	assignment.DesiredConfigurationRevision = snapshot.Revision
	assignment.DesiredConfigurationDigest = digest
	if _, err := orchestrator.Reconcile(t.Context(), ReconcileRequest{
		SchemaVersion: 1, Revision: 1, Runtimes: []RuntimeAssignment{assignment},
	}); err != nil {
		t.Fatal(err)
	}
	...
	configurationURL := controlServer.URL + "/orchestrator/v1/runtimes/alpha/configuration"
	request := ConfigurationSyncRequest{
		SchemaVersion: 1, PlacementEpoch: assignment.PlacementEpoch,
		Digest: digest, Configuration: snapshot,
	}
	response, body := controlRequest(t, http.MethodPut, configurationURL, request)
	if response.StatusCode != http.StatusOK {
		t.Fatalf("initial configuration sync = %d %s", response.StatusCode, body)
	}
	first, exists := supervisor.Application("alpha")
	if !exists || first == nil {
		t.Fatal("initial application is missing")
	}
	if err := supervisor.RemoveAccount(t.Context(), "alpha"); err != nil {
		t.Fatal(err)
	}
	second, err := supervisor.AddAccount(t.Context(), AccountConfig{
		ID: "alpha", BackgroundOnly: true, StartSession: false,
		ControlConfigurationRequired: true, ControlConfigurationReady: false,
	})
	if err != nil {
		t.Fatal(err)
	}
	...
	status := orchestrator.Status().Runtimes[0]
	if status.ConfigurationState != "pending" || status.AppliedConfigurationRevision != 0 ||
		status.AppliedConfigurationDigest != "" {
		t.Fatalf("replacement inherited a prior application acknowledgement: %+v", status)
	}
	response, body = controlRequest(t, http.MethodPut, configurationURL, request)
	if response.StatusCode != http.StatusOK || !bytes.Contains(body, []byte(`"configurationState":"ready"`)) {
		t.Fatalf("replacement configuration sync = %d %s", response.StatusCode, body)
	}
	stored, ok := second.Configuration.Section("scheduler")
	if !ok || !bytes.Equal(stored, snapshot.Sections["scheduler"]) {
		t.Fatalf("replacement scheduler configuration = %s", stored)
	}
	...
	if status.ConfigurationState != "ready" || status.AppliedConfigurationRevision != snapshot.Revision ||
		status.AppliedConfigurationDigest != digest {
		t.Fatalf("replacement did not acknowledge its own configuration: %+v", status)
	}
}
...
func TestOrchestratorKeepsTenantRuntimeAcrossGameDisconnectAndReconnectControl(t *testing.T) {
	supervisor, _, orchestrator, now := newTestOrchestrator(t)
	alpha := testAssignment("alpha", "tenant-one", 1, now.Add(10*time.Minute))
	...
	status := orchestrator.Status()
	if len(status.Runtimes) != 1 || status.Runtimes[0].OnDisconnect != "hold" {
		t.Fatalf("legacy release policy was not migrated to hold: %+v", status.Runtimes)
	}
	// Omitted, legacy release, and explicit hold assignments all canonicalize to
	// hold so the tenant API remains available without a game connection.
	bravo := testAssignment("bravo", "tenant-two", 1, now.Add(10*time.Minute))
	...
	if _, err := orchestrator.Reconcile(t.Context(), ReconcileRequest{SchemaVersion: 1, Revision: 2, Runtimes: []RuntimeAssignment{alpha, bravo, charlie}}); err != nil {
		t.Fatalf("canonical policy replay under a reused revision = %v", err)
	}
