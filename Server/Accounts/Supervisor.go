...
type AccountConfig struct {
	...
	BackgroundOnly bool
	StartSession   bool
	// ControlConfigurationRequired is set by hosted orchestration before the
	// application goroutines start, closing the startup window in which local
	// scheduler or automation writes could run ahead of the account snapshot.
	ControlConfigurationRequired bool
	ControlConfigurationReady    bool
...
func (supervisor *Supervisor) AddAccount(ctx context.Context, config AccountConfig) (*App.Application, error) {
	...
	application.SetControlConfigurationReady(
		config.ControlConfigurationRequired,
		config.ControlConfigurationReady,
	)
