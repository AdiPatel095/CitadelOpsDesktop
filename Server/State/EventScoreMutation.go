...
func (state GameState) LookupEventOccurrence(eventID int64) (EventOccurrence, bool) {
	...
	activity, activityFound := state.LookupEventActivity(eventID)
	if activityFound && !activity.OccurrenceEndsAt.IsZero() &&
		(endsAt.IsZero() || SameEventOccurrence(activity.OccurrenceEndsAt, endsAt)) {
		endsAt = activity.OccurrenceEndsAt.UTC()
	}
	if endsAt.IsZero() {
		return EventOccurrence{}, false
	}
	occurrence := EventOccurrence{EndsAt: endsAt.UTC()}
	if activityFound && SameEventOccurrence(activity.OccurrenceEndsAt, occurrence.EndsAt) {
		occurrence.ObservedFrom = activity.ObservedFrom.UTC()
	}
...
func (state *GameState) ReplaceEventInventory(inventory EventInventoryState) bool {
	...
	if inventory.GlobalEffects == nil {
		inventory.GlobalEffects = map[int64]GlobalEffectAvailability{}
	} else {
		inventory.GlobalEffects = cloneMap(inventory.GlobalEffects)
	}
	if inventory.GlobalEffectBoosterOffers == nil {
		inventory.GlobalEffectBoosterOffers = map[int64]GlobalEffectBoosterOffer{}
	} else {
		inventory.GlobalEffectBoosterOffers = cloneMap(inventory.GlobalEffectBoosterOffers)
	}
	if inventory.GlobalEffectBoosts == nil {
		inventory.GlobalEffectBoosts = map[int64]GlobalEffectBoostState{}
	} else {
