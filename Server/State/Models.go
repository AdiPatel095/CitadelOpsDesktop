...
type AllianceHelpRequestState struct {
	...
	OwnRecruitmentRequests           []RecruitmentAllianceHelpRequest `json:"ownRecruitmentRequests"`
	OwnRecruitmentObservedGeneration uint64                           `json:"ownRecruitmentObservedGeneration,omitempty"`
	PendingOtherListIDs              []int64                          `json:"pendingOtherListIds"`
	ObservedAt                       time.Time                        `json:"observedAt,omitempty"`
	OwnObservedGeneration            uint64                           `json:"ownObservedGeneration,omitempty"`
...
// and lifecycle. Completed requests remain useful for a short, bounded grace
// after the server's immediate AHD removal; pending requests have CompletedAt
// unset and must still be present (RemovedAt unset).
type RecruitmentAllianceHelpRequest struct {
	ListID         int64     `json:"listId"`
	CastleID       CastleID  `json:"castleId"`
	Progress       int       `json:"progress"`
	MaximumHelpers int       `json:"maximumHelpers"`
	ObservedAt     time.Time `json:"observedAt,omitempty"`
	CompletedAt    time.Time `json:"completedAt,omitempty"`
	RemovedAt      time.Time `json:"removedAt,omitempty"`
}

// RecruitmentAllianceHelpCompletionGrace is a product-level same-focus
// assumption. The live protocol proves P=3 completion and immediate AHD but
// exposes no grace/expiry field, so this intentionally conservative window is
// not represented as a server-derived duration.
const RecruitmentAllianceHelpCompletionGrace = 3 * time.Minute

// MaximumOutstandingHospitalAllianceHelpRequests is deliberately
// conservative. The game can reject a second T=2 request even when it targets
...
func OwnAllianceHelpStateCurrent(state GameState) bool {
	...
	if generation == 0 {
		// Preserve the zero-generation behavior used before a game session has
		// started. A real connected session always has a positive generation.
...
func OwnAllianceHelpListCurrent(state GameState) bool {
	...
	return state.Session.ChangedAt.IsZero() ||
		!state.AllianceHelpRequests.ObservedAt.Before(state.Session.ChangedAt)
}

func ReconcileOwnRecruitmentAllianceHelp(state *GameState, castleID CastleID, requested bool) bool {
	if state == nil || castleID <= 0 || state.Session.Generation == 0 {
		return false
	}
	changed := prepareOwnAllianceHelpGeneration(state)
	requests := &state.AllianceHelpRequests.RecruitmentCastleIDs
	for index, existingID := range *requests {
	...
	*requests = append(*requests, castleID)
	sort.Slice(*requests, func(left, right int) bool { return (*requests)[left] < (*requests)[right] })
	return true
}

func ReconcileOwnHospitalAllianceHelp(state *GameState, productionID int64, requested bool) bool {
	if state == nil || productionID <= 0 || state.Session.Generation == 0 {
		return false
	}
	changed := prepareOwnAllianceHelpGeneration(state)
	requests := &state.AllianceHelpRequests.HospitalProductionIDs
	for index, existingID := range *requests {
	...
	*requests = append(*requests, productionID)
	sort.Slice(*requests, func(left, right int) bool { return (*requests)[left] < (*requests)[right] })
	return true
}

func prepareOwnAllianceHelpGeneration(state *GameState) bool {
	if state == nil || state.Session.Generation == 0 ||
		state.AllianceHelpRequests.OwnObservedGeneration == state.Session.Generation {
		return false
	}
	state.AllianceHelpRequests.HospitalProductionIDs = []int64{}
	...
	state.AllianceHelpRequests.OwnRecruitmentObservedGeneration = 0
	state.AllianceHelpRequests.ObservedAt = time.Time{}
	state.AllianceHelpRequests.OwnObservedGeneration = state.Session.Generation
	return true
}
...
func HasOutstandingRecruitmentAllianceHelpRequest(state GameState, castleID CastleID) bool {
	...
	if state.Session.Generation > 0 &&
		...
		return RecruitmentAllianceHelpCovers(state, castleID, time.Now().UTC(), 0)
	}
	if OwnAllianceHelpStateCurrent(state) {
		...
		observedAt := state.AllianceHelpRequests.ObservedAt
		if !observedAt.IsZero() &&
			(state.Session.ChangedAt.IsZero() || !observedAt.Before(state.Session.ChangedAt)) {
			return false
		}
	}
	...
	if queue.ObservedAt.IsZero() ||
		!state.Session.ChangedAt.IsZero() && queue.ObservedAt.Before(state.Session.ChangedAt) {
		return false
	}
...
// RecruitmentAllianceHelpCovers reports whether exact current-session server
// lifecycle evidence covers another recruitment BUP through the supplied
// execution horizon. Pending P<maximum requests cover while retained. P>=max
...
func RecruitmentAllianceHelpCovers(
	...
	if castleID <= 0 || now.IsZero() || executionHorizon < 0 || state.Session.Generation == 0 ||
		state.AllianceHelpRequests.OwnRecruitmentObservedGeneration != state.Session.Generation {
		return false
	}
	safeThrough := now.Add(executionHorizon)
	for _, request := range state.AllianceHelpRequests.OwnRecruitmentRequests {
		if request.ListID <= 0 || request.CastleID != castleID || request.MaximumHelpers <= 0 ||
			request.ObservedAt.IsZero() ||
			!state.Session.ChangedAt.IsZero() && request.ObservedAt.Before(state.Session.ChangedAt) {
			continue
		}
		if request.Progress < request.MaximumHelpers {
		...
		if safeThrough.Before(request.CompletedAt.Add(RecruitmentAllianceHelpCompletionGrace)) {
			return true
		}
	}
	return false
...
// Khan movement PT and TT values are whole seconds. ArrivesAt retains the
// subsecond part of the local frame receipt time, so two independently
// observed movements can appear inverted by less than one second even though
// the wire data cannot establish that order.
const KhanArrivalOrderTolerance = time.Second

func KhanLaunchOvertakes(previousArrival, currentArrival time.Time) bool {
	if previousArrival.IsZero() || currentArrival.IsZero() {
		return false
	}
	return currentArrival.Add(KhanArrivalOrderTolerance).Before(previousArrival)
}
...
type KhanState struct {
	...
	LastTauntTriggeredEventEndsAt time.Time                         `json:"lastTauntTriggeredEventEndsAt,omitempty"`
