...
func planBeriCapacityRefresh(_ context.Context, input Intent.PlanningContext, arguments json.RawMessage) (Intent.Plan, error) {
	...
	return Intent.Plan{
		Claims: []string{
			"castle:" + castleID, "kingdom:" + strconv.FormatInt(int64(beriKingdomID), 10),
...
func (application *Application) verifyBeriCapacity(_ context.Context, arguments json.RawMessage) error {
	...
	if request.BeriCastleID <= 0 || request.RequestedAt.IsZero() {
		return fmt.Errorf("Berimond capacity verification requires a castle and request time")
	}
