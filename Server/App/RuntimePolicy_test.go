...
func TestExecutionGateFailsHostedWritesClosedUntilControlConfigurationIsReady(t *testing.T) {
	application := &Application{}
	application.SetControlConfigurationReady(true, false)
	err := application.executionGate(
		t.Context(), Intent.Request{Actor: "automation:autoKhan"},
		Intent.Plan{Effect: Intent.EffectWrite}, Intent.ExecutionBeforeClaims,
	)
	if !errors.Is(err, Intent.ErrPlanStale) {
		t.Fatalf("pending control configuration gate = %v", err)
	}
	if err := application.executionGate(
		t.Context(), Intent.Request{Actor: "ui"}, Intent.Plan{Effect: Intent.EffectRead},
		Intent.ExecutionBeforeClaims,
	); err != nil {
		t.Fatalf("pending control configuration blocked a read: %v", err)
	}
	application.SetControlConfigurationReady(true, true)
	err = application.executionGate(
		t.Context(), Intent.Request{Actor: "ui"},
		Intent.Plan{Effect: Intent.EffectWrite, Claims: []string{"configuration:scheduler"}},
		Intent.ExecutionBeforeClaims,
	)
	if !errors.Is(err, Intent.ErrPlanStale) {
		t.Fatalf("hosted runtime-local configuration write gate = %v", err)
	}
	if err := application.executionGate(
		t.Context(), Intent.Request{Actor: "ui"}, Intent.Plan{Effect: Intent.EffectWrite},
		Intent.ExecutionBeforeClaims,
	); err != nil {
		t.Fatalf("ready control configuration blocked a write: %v", err)
	}
	application.SetControlConfigurationReady(false, false)
	if err := application.executionGate(
		t.Context(), Intent.Request{Actor: "ui"}, Intent.Plan{Effect: Intent.EffectWrite},
		Intent.ExecutionBeforeClaims,
	); err != nil {
		t.Fatalf("desktop configuration was gated: %v", err)
	}
}

func TestBackgroundOnlyExecutionGateRejectsRuntimeLocalDashboardSavesAtRevisionZero(t *testing.T) {
	application := &Application{backgroundOnly: true}
	for _, name := range []string{"config.update", "session.background.prepare", "session.select_browser"} {
		err := application.executionGate(
			t.Context(), Intent.Request{Name: name, Actor: "ui-intent-console"},
			Intent.Plan{Effect: Intent.EffectWrite}, Intent.ExecutionBeforeClaims,
		)
		if !errors.Is(err, Intent.ErrPlanStale) {
			t.Fatalf("background-only %s gate = %v", name, err)
		}
	}
	if err := application.executionGate(
		t.Context(), Intent.Request{Name: "session.reconnect", Actor: "ui"},
		Intent.Plan{Effect: Intent.EffectExternal}, Intent.ExecutionBeforeClaims,
	); err != nil {
		t.Fatalf("background-only operational intent was blocked: %v", err)
	}
	if err := (&Application{}).executionGate(
		t.Context(), Intent.Request{Name: "config.update", Actor: "ui"},
		Intent.Plan{Effect: Intent.EffectWrite}, Intent.ExecutionBeforeClaims,
	); err != nil {
		t.Fatalf("desktop configuration intent was blocked: %v", err)
	}
}
...
func TestExecutionGateRechecksRageBoosterOnlyForAutoKhanAttackAdmission(t *testing.T) {
	configuration, err := Configuration.Open(t.TempDir(), map[string]json.RawMessage{
		"automation.autoKhan": json.RawMessage(`{"requireActiveRageBooster":true}`),
	})
	if err != nil {
		t.Fatal(err)
	}
	...
	if err := application.executionGate(
		t.Context(), Intent.Request{Actor: "automation:autoKhan"}, attackPlan, Intent.ExecutionBeforeClaims,
	); err == nil || !errors.Is(err, Intent.ErrPlanStale) || !strings.Contains(err.Error(), "boi ID 27") {
		t.Fatalf("inactive Auto Khan rage booster gate error = %v", err)
	}
	if err := application.executionGate(
		t.Context(), Intent.Request{Actor: "automation:autoKhan"}, attackPlan, Intent.ExecutionBeforeStep,
	); err != nil {
		t.Fatalf("post-launch Auto Khan steps were blocked by the rage booster gate: %v", err)
	}
	if err := application.executionGate(
		t.Context(), Intent.Request{Actor: "automation:autoKhan"}, Intent.Plan{Effect: Intent.EffectWrite},
		Intent.ExecutionBeforeClaims,
	); err != nil {
		t.Fatalf("Khan taunt or defense work was blocked by attack booster gate: %v", err)
	}
	otherAttack := attackPlan
	otherAttack.Admission = &Intent.Admission{Class: Intent.AdmissionAttackLaunch, Module: "autoNomad"}
	if err := application.executionGate(
		t.Context(), Intent.Request{Actor: "automation:autoNomad"}, otherAttack, Intent.ExecutionBeforeClaims,
	); err != nil {
		t.Fatalf("another attack module was blocked by Auto Khan booster gate: %v", err)
	}

	_, err = application.State.Apply(func(state *State.GameState) ([]string, bool, error) {
		state.Market.Boosters[GameData.KhanRagePointsBoosterID] = State.MarketBoosterState{
	...
	if err := application.executionGate(
		...
		t.Fatalf("active Khan Rage booster was blocked: %v", err)
	}
	if err := application.executionGate(
		t.Context(), Intent.Request{Actor: "automation:autoKhan"}, attackPlan, Intent.ExecutionBeforeStep,
	); err != nil {
		t.Fatalf("active Khan Rage booster was blocked before dispatch: %v", err)
	}
}

func TestKhanAttackResolverRechecksLatestRageBoosterRequirement(t *testing.T) {
	configuration, err := Configuration.Open(t.TempDir(), map[string]json.RawMessage{
		"automation.autoKhan": json.RawMessage(`{"requireActiveRageBooster":true}`),
	})
	if err != nil {
