...
var userFacingActivityIdentifierPatterns = []struct {
	...
	{regexp.MustCompile(`(?i)\bproduction job (?:id )?[0-9]+\b`), "the production queue"},
	{regexp.MustCompile(`(?i)\bshop table (?:id )?[0-9]+\b`), "the current shop"},
	{regexp.MustCompile(`(?i)\boffer (?:id )?[0-9]+\b`), "the selected offer"},
	{regexp.MustCompile(`(?i)\b(?:battle |spy )?report (?:id )?[0-9]+\b`), "the selected report"},
}

var userFacingTechnicalFailurePattern = regexp.MustCompile(
	`(?i)(?:\b(?:opcode|payload|resolver|dependency|intent|operation|revision|generation|cursor|json field|response code)\b|` +
...
func userFacingFailureReason(value string) string {
	...
	case strings.Contains(lower, "no commander is available"), strings.Contains(lower, "no commander was available"),
		strings.Contains(lower, "no available commander"):
		return "no eligible commander was available"
	...
	case strings.Contains(lower, "response code"):
		...
		if marker := strings.Index(lower, unsuccessfulMarker); marker >= 0 {
			...
			if officialReason != "" {
				return userFacingActivityText(officialReason)
			}
		}
		return "the game rejected the action"
	...
	if marker := strings.Index(lower, unsuccessfulMarker); marker >= 0 {
		...
		if reason != "" {
			return userFacingActivityText(reason)
		}
	}
	if userFacingTechnicalFailurePattern.MatchString(reason) {
		return "an internal app error prevented the action"
	}
	return userFacingActivityText(reason)
}
...
func userFacingActivityText(value string) string {
	...
	value = strings.NewReplacer(
		"game websocket", "game connection",
		"Game websocket", "Game connection",
	).Replace(value)
