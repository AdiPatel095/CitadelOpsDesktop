...
func validateKhanTauntContext(
	...
	if !active || score.RemainingSec <= 0 || score.ObservedAt.IsZero() || invasionRemainingSeconds(score, now) == 0 {
		return fmt.Errorf("%w: the Nomad event is no longer active", Intent.ErrPlanStale)
	}
	...
	if !occurrenceFound || !State.SameEventOccurrence(request.EventEndsAt, occurrence.EndsAt) {
		return fmt.Errorf("%w: the active Khan event occurrence changed", Intent.ErrPlanStale)
	}
	main, exists := gameState.Castles[request.MainCastleID]
	if !exists || main.KingdomID != 0 || main.SlotType != 1 {
		return fmt.Errorf("%w: Khan taunt target must be the Great Empire main castle", Intent.ErrPlanStale)
	}
	...
	if !exists || target.TypeID != khanCampTypeID {
		return fmt.Errorf("%w: the active type-35 Khan camp changed or is unavailable", Intent.ErrPlanStale)
	}
	if gameData == nil {
	...
	if !found || camp.EventID != khanEventID || camp.AreaTypeID != khanCampTypeID ||
		...
		return fmt.Errorf("%w: the authoritative Khan rage cap is unavailable or changed", Intent.ErrPlanStale)
	}
	khan := gameState.Khan
	if khan.RageCampID != request.RageCampID ||
		...
		return fmt.Errorf("%w: the Khan rage bar is no longer ready for this taunt", Intent.ErrPlanStale)
	}
	if request.KhanGuard.MainCastleID != request.MainCastleID {
		return fmt.Errorf("Khan taunt safety guard does not match the main castle")
	}
...
func khanAttackContext(
	...
	if request.MaxRageChain < 0 {
		return khanAttackRequest{}, State.CastleState{}, State.MapObservation{}, fmt.Errorf("maximum Khan rage chain cannot be negative")
	}
	...
	if request.EventEndsAt.IsZero() || !occurrenceFound ||
		...
		return khanAttackRequest{}, State.CastleState{}, State.MapObservation{}, fmt.Errorf("the active Khan event occurrence changed")
	}
	if request.MaxRageChain > 0 {
...
func (*Application) rejectUnconfirmedKhanTauntDispatch(_ context.Context, _ json.RawMessage) error {
	return fmt.Errorf("%w: Khan taunt dispatch did not await an accepted GAM response", Intent.ErrPlanStale)
}
