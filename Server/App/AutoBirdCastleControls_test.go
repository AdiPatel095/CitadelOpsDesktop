...
func TestCastleBirdBatchGuardPreservesPresetPeriodBoundary(t *testing.T) {
	now := time.Now().UTC()
	request := autoBirdCycleRequest{SourceCastleID: 10, PresetValidUntil: now}
	if err := validateAutoBirdControl(State.NewGameState(), request, now); !errors.Is(err, Intent.ErrPlanStale) {
		t.Fatalf("expired preset accepted by batch guard: %v", err)
	}
}
