...
func TestCooperativeCaptureBindsIdentityForTargetedRefresh(t *testing.T) {
	...
	// its settle window before expecting this sole participant's lease.
	worldMaps.AcquireStormScan("acct-test", worldID, stormIntentKingdomID, startedAt)
	assignment := worldMaps.AcquireStormScan(
		"acct-test", worldID, stormIntentKingdomID, startedAt.Add(3*time.Second),
	)
