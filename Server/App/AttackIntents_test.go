...
func TestBuildAttackSetupFillsPartialHigherTierWithLowerTroopFamilyTier(t *testing.T) {
	gameData, err := GameData.DecodeStore([]byte(`{
	...
	preset := AttackPresets.Preset{
		ID: "family", Name: "Family fill", UseTroopFamilies: true,
	...
	built, err := buildAttackSetup(invasionAttackSetup(preset), source, gameData)
	if err != nil {
		t.Fatal(err)
	}
	units := built.Waves[0].Middle.Units
	if units[0] != (attackPair{4, 40}) || units[1] != (attackPair{3, 60}) {
		t.Fatalf("family-filled CRA units = %#v, want 40 tier-4 then 60 tier-3", units)
	}
}
...
func TestRiftReplayUsesAccountOwnedTemplateWithEmptyRuntimeRiftState(t *testing.T) {
	gameState, launch := configuredRiftReplayFixture()
	application := configuredRiftReplayApplication(t, RiftTemplates.Document{
		Version: 1, Launches: map[string]State.RiftLaunch{launch.ID: launch},
		DeletedLaunchIDs: map[string]int64{},
	})

	plan, err := application.planRiftReplay(
		context.Background(), Intent.PlanningContext{State: gameState},
		json.RawMessage(`{"launchId":"rift-safe"}`),
	)
	if err != nil {
		t.Fatal(err)
	}
	if len(plan.Steps) != 4 || plan.Steps[2].Opcode != "cra" {
		t.Fatalf("configured Rift replay plan = %#v", plan)
	}
	var body map[string]json.RawMessage
	if err := json.Unmarshal(plan.Steps[2].Command.Payload, &body); err != nil {
		t.Fatal(err)
	}
	if rawMapInt(body, "TX") != int64(launch.TargetX) || rawMapInt(body, "TY") != int64(launch.TargetY) {
		t.Fatalf("configured target body = %s", plan.Steps[2].Command.Payload)
	}
}

func TestRiftReplayTombstoneBlocksRuntimeFallback(t *testing.T) {
	gameState, launch := configuredRiftReplayFixture()
	gameState.Rift.Launches[launch.ID] = launch
	application := configuredRiftReplayApplication(t, RiftTemplates.Document{
		Version: 1, Launches: map[string]State.RiftLaunch{},
		DeletedLaunchIDs: map[string]int64{launch.ID: time.Now().UTC().UnixMilli()},
	})
...
func TestRiftReplayRejectsMalformedOrUnsafeConfiguredTemplate(t *testing.T) {
	tests := []struct {
		name     string
		raw      json.RawMessage
	...
	}{
		{
			...
			raw:      json.RawMessage(`{"version":2,"launches":{},"deletedLaunchIds":{}}`),
			contains: "unsupported Rift template document version",
		},
		{
			name: "arbitrary CRA field",
			raw: configuredRiftReplayDocument(t, func(candidate *State.RiftLaunch) {
				candidate.Body = json.RawMessage(`{"LID":5,"SX":1,"SY":2,"TX":10,"TY":20,"KID":0,"AV":1,"HBW":-1,"PTT":1,"A":[{"L":{"U":[[300,11]]}}],"TARGET_OVERRIDE":99}`)
			}),
			contains: "unsupported CRA field",
		},
		{
			name: "non Rift current target",
			raw:  configuredRiftReplayDocument(t, nil),
			mutate: func(state *State.GameState) {
				state.Map[0]["10:20"] = State.MapObservation{KingdomID: 0, X: 10, Y: 20, TypeID: 4}
			},
			contains: "not a current known Rift target type 43",
		},
		{
			name: "unowned formation",
			raw: configuredRiftReplayDocument(t, func(candidate *State.RiftLaunch) {
				candidate.Body = json.RawMessage(`{"LID":5,"SX":1,"SY":2,"TX":10,"TY":20,"KID":0,"AV":1,"HBW":-1,"PTT":1,"A":[{"L":{"U":[[300,99]]}}]}`)
			}),
			contains: "require 99",
		},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			state, _ := configuredRiftReplayFixture()
			if test.mutate != nil {
				test.mutate(&state)
			}
			configuration, err := Configuration.Open(t.TempDir(), map[string]json.RawMessage{
				RiftTemplates.ConfigurationSection: test.raw,
			})
			if err != nil {
				t.Fatal(err)
			}
			...
			_, err = application.planRiftReplay(
				context.Background(), Intent.PlanningContext{State: state},
				json.RawMessage(`{"launchId":"rift-safe"}`),
			)
			if err == nil || !strings.Contains(err.Error(), test.contains) {
				t.Fatalf("configured replay error = %v, want %q", err, test.contains)
			}
		})
	}
}

func TestRiftReplayConfiguredTemplatePrecedesRuntimeCapture(t *testing.T) {
	gameState, configured := configuredRiftReplayFixture()
	runtime := configured
	runtime.TargetX = 99
	runtime.Body = json.RawMessage(`{"LID":5,"SX":1,"SY":2,"TX":99,"TY":20,"KID":0,"AV":1,"HBW":-1,"PTT":1,"A":[{"L":{"U":[[300,1]]}}]}`)
	gameState.Rift.Launches[configured.ID] = runtime
	application := configuredRiftReplayApplication(t, RiftTemplates.Document{
		Version: 1, Launches: map[string]State.RiftLaunch{configured.ID: configured},
		DeletedLaunchIDs: map[string]int64{},
	})
	...
	if got := rawMapInt(body, "TX"); got != 10 {
		t.Fatalf("replay target X = %d, want configured value 10", got)
	}
}
...
func configuredRiftReplayFixture() (State.GameState, State.RiftLaunch) {
	...
	gameState.Castles[1] = State.CastleState{
		...
		Units: State.CastleUnits{Stationed: map[State.UnitID]int64{300: 20}},
	}
	gameState.Map[0] = map[string]State.MapObservation{
		"10:20": {KingdomID: 0, X: 10, Y: 20, TypeID: riftMapTypeID},
	}
	launch := State.RiftLaunch{
		ID: "rift-safe", SavedAtUnix: 1, CommanderID: 5,
		SourceX: 1, SourceY: 2, TargetX: 10, TargetY: 20, KingdomID: 0,
		AttackValid: 1, WaveCount: 1, UseTravelFeather: true,
		Body: json.RawMessage(`{"LID":5,"SX":1,"SY":2,"TX":10,"TY":20,"KID":0,"AV":1,"HBW":-1,"PTT":1,"A":[{"L":{"U":[[300,11]]}}]}`),
	}
	return gameState, launch
}

func configuredRiftReplayApplication(t *testing.T, document RiftTemplates.Document) *Application {
	t.Helper()
	...
	configuration, err := Configuration.Open(t.TempDir(), map[string]json.RawMessage{
		RiftTemplates.ConfigurationSection: raw,
	})
	if err != nil {
		t.Fatal(err)
	}
...
func configuredRiftReplayDocument(t *testing.T, mutate func(*State.RiftLaunch)) json.RawMessage {
	t.Helper()
	_, launch := configuredRiftReplayFixture()
	if mutate != nil {
		mutate(&launch)
	}
	raw, err := json.Marshal(RiftTemplates.Document{
		Version: 1, Launches: map[string]State.RiftLaunch{launch.ID: launch},
		DeletedLaunchIDs: map[string]int64{},
	})
	if err != nil {
