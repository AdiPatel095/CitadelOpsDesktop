...
func TestFeatureActivitiesRecordsOneCompletedAction(t *testing.T) {
	...
	if activities[0].severity != "INFO" || activities[0].event != "QUEUE" ||
		activities[0].detail != "Queued 5 stacks of 40 tools at DesertTown" {
...
func TestFeatureActivitiesRecordsEachAttackInChain(t *testing.T) {
	...
	if activities[0].detail != "Launched camp attack with a commander (1 of 2)" ||
		activities[1].detail != "Launched camp attack with a commander (2 of 2)" {
...
func TestFeatureActivitiesRecordsOneUserFacingFailure(t *testing.T) {
	...
	if strings.Contains(activities[0].detail, "intent=") || strings.Contains(activities[0].detail, "operation=") ||
		activities[0].detail != "Could not buy construction-item package from Baltimore: the shop offer expired" {
...
func TestFeatureActivitiesMarksAttackInventoryGateAsWarning(t *testing.T) {
	...
	if activities[0].detail != "Could not launch an attack against Storm fort at 600:555 with Trial: there were not enough eligible troops available" {
...
func TestFeatureActivitiesHideLegacyIdentifiersAndProtocolFields(t *testing.T) {
	receipt := Intent.Receipt{
		Intent: "equipment.equip", Status: Intent.StatusFailed,
		Error: "equipment resolver payload WID=501 was rejected",
		Plan: &Intent.Plan{
			Effect:  Intent.EffectWrite,
			Summary: "Equip Relic Armor (equipment ID 40) on The Bold (commander ID 20)",
			Steps:   []Intent.Step{{Opcode: "eqi"}},
		},
	}
	activities := featureActivities(receipt)
	if len(activities) != 1 {
	...
	for _, forbidden := range []string{"ID 40", "ID 20", "WID", "payload", "resolver"} {
		if strings.Contains(detail, forbidden) {
			t.Fatalf("user activity %q contains %q", detail, forbidden)
		}
	}
	if detail != "Could not equip Relic Armor on The Bold: an internal app error prevented the action" {
		t.Fatalf("user activity = %q", detail)
	}
}
