...
func TestPlanCastleFocusTreatsCurrentTargetAsSatisfied(t *testing.T) {
	...
	if len(plan.Steps[0].StaleCodes) != 1 || plan.Steps[0].StaleCodes[0] != 175 {
		t.Fatalf("explicit focused castle refresh stale codes = %#v, want [175]", plan.Steps[0].StaleCodes)
	}
}

func TestPlanCastleRefreshRejectsCurrentRetainedUnfocusableCastle(t *testing.T) {
	now := time.Date(2026, 8, 29, 12, 0, 0, 0, time.UTC)
	gameState := State.NewGameState()
	gameState.Session.ChangedAt = now.Add(-time.Minute)
	gameState.KingdomTransport.ObservedAt = now
	gameState.KingdomTransport.Unlocks[10] = State.KingdomTransportUnlock{KingdomID: 10, Created: false}
	gameState.Castles[77] = State.CastleState{ID: 77, KingdomID: 10}

	_, err := planCastleFocus(
		t.Context(), Intent.PlanningContext{State: gameState}, json.RawMessage(`{"castleId":77,"refresh":true}`),
	)
	if !errors.Is(err, Intent.ErrPlanStale) {
		t.Fatalf("unfocusable castle refresh error = %v, want stale plan", err)
	}
}

func TestPlanCastleFocusAcceptsUnlockedStormSeasonalRepresentation(t *testing.T) {
	now := time.Date(2026, 8, 30, 2, 17, 0, 0, time.UTC)
	gameState := State.NewGameState()
	gameState.Session.ChangedAt = now.Add(-time.Minute)
	gameState.KingdomTransport.ObservedAt = now
	gameState.KingdomTransport.Unlocks[4] = State.KingdomTransportUnlock{
		KingdomID: 4, Unlocked: true, Created: false, Stage: 1,
	}
	gameState.Castles[3849] = State.CastleState{
		ID: 3849, KingdomID: 4, SlotType: 12, X: 615, Y: 552,
	}

	plan, err := planCastleFocus(
		t.Context(), Intent.PlanningContext{State: gameState}, json.RawMessage(`{"castleId":3849}`),
	)
	if err != nil {
		t.Fatal(err)
	}
	if len(plan.Steps) != 1 || plan.Steps[0].Opcode != "jaa" || plan.Steps[0].AwaitOpcode != "jaa" ||
		string(plan.Steps[0].Command.Payload) != `{"PX":615,"PY":552,"KID":4}` ||
		plan.Steps[0].ResponseBarrier != Intent.ResponseBarrierCommitted {
		t.Fatalf("Storm focus plan = %#v, want one committed JAA", plan.Steps)
	}
}
