...
func TestKingdomSkipPlannerRequiresObservedInventory(t *testing.T) {
	...
	if len(plan.Steps) != 4 || plan.Steps[0].Opcode != "kpi" ||
		plan.Steps[1].Action != "resources.kingdom.verify_pending" ||
		plan.Steps[2].Opcode != "msk" || string(plan.Steps[2].Command.Payload) != `{"KID":"1","MST":"MS5","TT":"2"}` {
		t.Fatalf("unexpected reconciled skip plan: %+v", plan)
	}
	if plan.Steps[0].ResumePolicy != Intent.ResumeRebuild ||
		plan.Steps[1].ResumePolicy != Intent.ResumeRebuild ||
		len(plan.Steps[2].StaleCodes) != 1 || plan.Steps[2].StaleCodes[0] != 182 ||
		plan.Steps[3].Action != timeSkipConsumeAction ||
...
func TestKingdomSkipGuardUsesRefreshedPendingState(t *testing.T) {
	...
	if err := application.verifyKingdomResourceTransportPending(t.Context(), arguments); err != nil {
		t.Fatalf("observed pending transport was rejected: %v", err)
	}
	_, err := application.State.ApplyComponents(State.Components(State.ComponentKingdomTransport), func(current *State.GameState) ([]string, bool, error) {
		current.KingdomTransport.Pending = nil
		current.KingdomTransport.ObservedAt = now.Add(time.Second)
		return []string{"kingdom-transport"}, true, nil
	})
	if err != nil {
		t.Fatal(err)
	}
	if err := application.verifyKingdomResourceTransportPending(t.Context(), arguments); !errors.Is(err, Intent.ErrPlanStale) {
		t.Fatalf("completed transport guard error = %v", err)
	}
}

func TestKingdomSkipPlannerClassifiesMissingTransportAsStale(t *testing.T) {
	gameState := State.NewGameState()
	gameState.Player.Currencies[50] = 2
	_, err := planKingdomResourceSkip(t.Context(), Intent.PlanningContext{
		State: gameState, GameData: resourceIntentGameData(t),
	}, json.RawMessage(`{"targetKingdomId":4,"timeSkipId":"MS5"}`))
...
func TestImmediateSettlementDoesNotAssumeKingdomTransportDuration(t *testing.T) {
	gameData := resourceIntentGameData(t)
	gameState := State.NewGameState()
	donor := resourceIntentCastle(10, 0, 100, 200)
	donor.Resources[3] = State.ResourceBalance{Amount: 50_000}
	...
	if len(plan.Steps) != 10 || plan.Steps[5].Opcode != "msk" || plan.Steps[7].Opcode != "kpi" ||
		plan.Steps[9].Action != "resources.kingdom.complete_workflow" {
		t.Fatalf("state-gated settlement steps = %#v", plan.Steps)
	}
}
