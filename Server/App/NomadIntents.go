...
func validateNomadCampPresetInventory(
	...
	if len(commanderIDs) == 0 {
		return nil, fmt.Errorf("camp attack requires at least one commander")
	}
	...
	for _, commanderID := range commanderIDs {
		...
		if err != nil {
			return nil, fmt.Errorf("resolve camp preset %q troop families for commander %d: %w", preset.Name, commanderID, err)
		}
		if shortage != nil {
			return nil, fmt.Errorf(
		...
		if _, err := buildAttackSetup(invasionAttackSetup(resolved), source, gameData); err != nil {
			return nil, fmt.Errorf("build camp preset %q for commander %d: %w", preset.Name, commanderID, err)
		}
