...
func TestPlanBeriCapacityRefreshScansExactDonorWithoutChangingFocus(t *testing.T) {
	gameState := State.NewGameState()
	gameState.Castles[100] = State.CastleState{ID: 100, KingdomID: 0}
	gameState.Castles[900] = State.CastleState{ID: 900, KingdomID: beriKingdomID, Focused: true}
	plan, err := planBeriCapacityRefresh(
		t.Context(), Intent.PlanningContext{State: gameState},
		json.RawMessage(`{"beriCastleId":900,"sourceCastleId":100}`),
	)
	if err != nil {
		t.Fatal(err)
	}
	if len(plan.Steps) != 3 || plan.Steps[0].Opcode != "dcl" ||
		plan.Steps[1].Opcode != "fuc" || plan.Steps[2].Action != "beri.capacity.verify" {
		t.Fatalf("unexpected capacity refresh plan: %#v", plan.Steps)
	}
	if string(plan.Steps[0].Command.Payload) != `{"CD":1}` {
		t.Fatalf("donor scan does not use the captured castle-detail payload: %s", plan.Steps[0].Command.Payload)
	}
	if slices.Contains(plan.Claims, "castle-focus") {
		t.Fatalf("donor inventory scan changes castle focus: %#v", plan.Claims)
	}
	for _, claim := range []string{"castle:100", "castle:900", "kingdom:10", "beri-capacity:900"} {
		if !slices.Contains(plan.Claims, claim) {
			t.Fatalf("capacity refresh is missing claim %q: %#v", claim, plan.Claims)
		}
	}
	var request beriCapacityRefreshRequest
	if err := json.Unmarshal(plan.Steps[2].ActionArguments, &request); err != nil {
		t.Fatal(err)
	}
	if request.BeriCastleID != 900 || request.SourceCastleID != 100 || request.RequestedAt.IsZero() {
		t.Fatalf("capacity verification request = %#v", request)
	}

	observedAt := request.RequestedAt.Add(time.Millisecond)
...
func TestAllOtherBeriPhasesPassProductionResourceAdmission(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			...
			arguments: json.RawMessage(`{"beriCastleId":900,"sourceCastleId":100}`),
			state: func() State.GameState {
				gameState := State.NewGameState()
				gameState.Castles[100] = State.CastleState{ID: 100, KingdomID: 0}
				gameState.Castles[900] = State.CastleState{ID: 900, KingdomID: beriKingdomID}
...
func TestPlanBeriTowerAttackUsesFNTAndOmitsADIAndGAS(t *testing.T) {
	...
	arguments, _ := json.Marshal(beriTowerAttackRequest{
		...
		DailyAttackLimit: 1000,
	})
	gameState.DailyAttacks = State.DailyAttackState{Count: 1000, ServerThreshold: 3500, ObservedAt: now}
	blockedPlan, err := planBeriTowerAttack(
		t.Context(), Intent.PlanningContext{State: gameState, GameData: gameData}, arguments,
	)
	if err != nil || len(blockedPlan.Steps) != 0 || !strings.Contains(blockedPlan.Summary, "1000 / 1000") {
		t.Fatalf("reached daily cap plan = %#v err=%v", blockedPlan, err)
	}
	gameState.DailyAttacks = State.DailyAttackState{Count: 999, ServerThreshold: 3500, ObservedAt: now.Add(time.Second)}
	...
	var focus, gaa, guards, dailyGuards, resolvers, analyticsCaptures int
	var guardArguments json.RawMessage
	var launchDependencies *Intent.CommandDependencyRequest
	var dailyGuardIndex, gaaIndex, launchIndex int
	...
	if focus != 2 || gaa != 1 || guards != 1 || dailyGuards != 1 || resolvers != 1 || analyticsCaptures != 1 {
		t.Fatalf(
			"unexpected cached attack steps: focus=%d gaa=%d guards=%d daily=%d resolvers=%d analytics=%d steps=%#v",
			focus, gaa, guards, dailyGuards, resolvers, analyticsCaptures, plan.Steps,
		)
	}
	if dailyGuardIndex >= gaaIndex || gaaIndex >= launchIndex {
		t.Fatalf("daily limit guard must run before target refresh and CRA: daily=%d gaa=%d launch=%d", dailyGuardIndex, gaaIndex, launchIndex)
	}
