...
var knownChannels = []Channel{
	...
	{ID: ChannelActivity, Label: "All Activity", Description: "Completed Citadel actions and issues that may need your attention."},
...
var unsafeFeatureActivityDetail = regexp.MustCompile(
	`(?i)(?:\([^()]{0,64}\bID\s+[0-9]+\)|` +
		`\b(?:construction[- ]item|crafting recipe|building(?: instance| definition| operation)?|` +
		`item|unit|troop|tool|commander|castellan|equipment|gem|movement|player|alliance|` +
		`resource|currency|package|kingdom|difficulty|event|effect)\s+(?:ID\s+)?[0-9]+\b|` +
		`\b(?:intent|operation|itemId|unitId|castleId|commanderId|AID|CID|KID|LID|OID|PID|SID|TID|WID|WOD|AMT)\s*[:=]|` +
		`\b(?:opcode|payload|resolver|revision|generation|websocket|CRA|GAA|JAA|SBP)\b|` +
		`%xt%|\{\s*"[A-Za-z0-9_]+"\s*:)`,
)
...
func (store *Store) RecordFeatureActivity(actor string, intent string, severity string, event string, detail string) {
	...
	if channel != "" && severity == "INFO" && event == "ATTACK" {
...
func storedFeatureActivityDetail(severity string, event string, detail string) string {
	...
	if severity == "INFO" {
		switch event {
		case "ALLIANCE HELP":
			return "Completed an alliance-help action"
		case "ATTACK":
			return "Launched an attack"
		case "BUILDING", "CONSTRUCTION":
			return "Completed a building action"
		case "CRAFTING":
			return "Completed a crafting action"
		case "DEFENSE":
			return "Updated the defense setup"
		case "EQUIPMENT":
			return "Completed an equipment action"
		case "ESPIONAGE":
			return "Sent spies"
		case "HOSPITAL":
			return "Completed a hospital action"
		case "PURCHASE":
			return "Completed a purchase"
		case "QUEUE":
			return "Added production to the queue"
		case "TIME SKIP":
			return "Applied a time skip"
		case "TRANSPORT":
			return "Completed a transport"
		default:
			return "Completed an action"
		}
	}
	return "Could not complete the action. Check the feature status for next steps."
}
...
func (store *Store) attackLaunchCounts(now time.Time, window time.Duration) map[string]int {
	...
	for _, channel := range attackFeatureChannels {
		...
		firstRetained := 0
		for firstRetained < len(launches) && launches[firstRetained].Before(retentionCutoff) {
			firstRetained++
		}
		if firstRetained > 0 {
			launches = append([]time.Time(nil), launches[firstRetained:]...)
		...
		counts[channel] = len(launches) - firstIncluded
	}
	return counts
}
...
// UserFacingTail returns only activity channels safe for the dashboard. Full
// command and websocket captures remain available to local support diagnostics
// but can never be selected by, or fetched from, the user-facing API.
func (store *Store) UserFacingTail(channel string, limit int) ([]string, bool) {
	if store == nil || !isKnownChannel(channel) || isDiagnosticChannel(channel) {
		return []string{}, false
	}
	return store.Tail(channel, limit), true
}
