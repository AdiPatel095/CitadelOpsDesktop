...
| `presetId` | Saved attack formation whose troop proportions guide transfers |
| `minTroopsToTransfer` | Minimum free-capacity threshold before starting a transfer |
...
1. **No owned Berimond camp observed or attack preset selected** — `waiting`.
...
5. **Ready** — choose one underrepresented Food-fed preset troop type and
   submit a guarded `beri.transfer` up to its proportional deficit, donor stock,
   and refreshed per-unit capacity.
...
- **Capacity threshold.** The configured minimum gates the reported free
  capacity; a proportional shipment may be smaller than that threshold.
- **Preset proportions.** Troop weights sum positive wave and courtyard troop
  slots, including repeated unit IDs. The policy uses deterministic integer
  rounding against current camp stock plus free capacity and refreshes after
  each confirmed shipment; missing donor stock waits rather than skewing the
  selected mix. Family presets resolve from fresh camp and donor inventory.
...
- **Validated batch.** `beri.transfer` is an `EffectLaunch` intent that
  validates the batch before an optional selected time skip is applied, so the
  skip is not spent on a transfer the game rejects.
- **Automatic camp and fixed wire context.** The owned Berimond camp is
  discovered from kingdom 10 state. The transfer intent uses its observed
  model id for ownership and capacity guards, while KUT sends `CID: -1`.
  Saved legacy castle-id overrides are ignored.
